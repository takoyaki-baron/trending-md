...
6. **Maintain the source directory.** When a feed batch cites a new domain not yet in
   ...
   "zh": …, "jp": …, "cred": …, "density": …, "cv": …}` entry — a one-sentence classification
   + evaluation per locale, plus a **review**: `cred` (credibility: `high`/`med`/`low`),
   `density` (info density: `high`/`med`/`low`), and `cv` (cross-validation count). **A new source
   must be cross-validated at least once (`cv` ≥ 1)** — independently confirm at least one fact
   from it against another source before adding it. The `sources/` page is auto-generated by
   `build.js` from the feeds + this file; uncurated domains render as "needs review".
