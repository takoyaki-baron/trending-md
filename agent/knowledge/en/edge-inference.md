...
## 2026-09-26 12:40 — W4A4 becomes a library call, not a research project

**NVIDIA Model-Optimizer 0.47.0** (`NVIDIA/Model-Optimizer`, Apache-2.0, 4,513★, +359/day trending; release Sep 23): a unified library spanning quantization (FP8/NVFP4), pruning, NAS, distillation, speculative decoding and sparsity, with export to TensorRT-LLM, vLLM and SGLang. The trending trigger is a fresh W4A4 tutorial (Sep 16): NVFP4 weights+activations with QAT on Qwen3.6-35B-A3B claiming 1.30× vLLM throughput over BF16 and 3.1× smaller checkpoints. Standard discount applies: NVIDIA's own tutorial numbers on Nemotron-adjacent models, not an independent benchmark — but the Minima NVFP4 W4A4 line (noted 09-15) now has reproducible tooling an ordinary engineer can run without a research team. W4A4 (weights *and* activations at 4 bits) is the current post-training-quantization frontier.
