...
- **Qwen3.8 27B 上 Cerebras 达约 1,500 tok/s（9 月 3 日，HN 250 分）。** 开放权重 Apache-2.0 稠密 27B
  （Qwen 8 月发布）以晶圆级速度服务：~1,500 tokens/s，64k/128k 上下文；同目录里 `gpt-oss-120b` 跑约
  3,000 tok/s。HN 讨论串里全是本地推理操作者在按自己的 tok/s 预算做算术。星号：这是 Cerebras 硬件，GPU
  节点不可复现。在本文件地图中的位置：agent 循环瓶颈在输出 token，1,500 tok/s 让长推理链的成本无关紧要
  ——*前提是*你接受晶圆级作为服务层；它是 M4 Pro 蓝图（自有硬件）与 WebLLM（浏览器）之外的服务速度端。

## 2026-09-05 04:03

- **NVIDIA 收购 Hugging Face 之后的 llama.cpp 前途（治理笔记）。** ggml.ai——Georgi Gerganov 与 llama.cpp
  创始团队——于 2 月加入 Hugging Face，并留下书面承诺：项目"一如既往保持开放与社区驱动"、"将继续 100%
  开源"、社区对技术与架构决策保持自主控制。NVIDIA 约 129 亿美元收购 HF 已成交并覆盖该团队，Gerganov 公开
  重申承诺——承诺是否守得住的考验才刚刚开始。社区在公告帖本身提出的担忧：美国司法辖区、所有权清晰度、
...
## Minima:全部线性层 W4A4,连循环层也不例外(09-06 04:03)

- **Minima AI(arXiv 2609.04098)对一个混合架构 27B 模型的全部 496 个线性层施加 NVFP4 W4A4**(16 个注意力层 + 48
  个 Gated DeltaNet 循环层)——量化工作通常会豁免脆弱部分——报告相对 BF16 的 5 任务平均差值为 −0.52(MMLU-Pro、
  GSM8K、AIME'25、GPQA-Diamond、LiveCodeBench、RULER 至 64K)。机制发现比头条更有意思:gate 投影把约 11% 的 GEMM
  误差转化为约 2% 的输出误差,且 delta-rule 递推在 32K token 内保持注入噪声平稳——**人人默认脆弱的循环那一半,
  对量化是稳定的**。最小配方 17.5 GiB,prefill 提速 +14–19%;checkpoint 公开(`minima-ai/mnma_qwen3.8_27b_nvfp4`)。
  作者自己的限制:单一架构(不做泛化声明),32K 处的困惑度差距只是"随位置缩小"而非消失,仅测了 NVFP4/FP8——而
  "在种子噪声范围内"是厂商对一个小的自报退化的措辞。如果循环层能在 4-bit 权重*与*激活下存活,混合 LLM 最后一个
  被豁免的组件就此倒下,sub-20GiB 的 27B 服务有了成文配方。

## 推测解码的反面案例由厂商自己印出来 (09-08)

vLLM 博客(AMD + Embedded LLM 团队,8 月 23 日;HN 触发 9 月 7 日,118 分)在 Instinct MI300X/MI355X + ROCm 上走了五种草稿方法——原生 MTP、Gemma 4 MTP、EAGLE-3、DFlash、DSpark。实测峰值:**2.83×** 输出 token 吞吐(Qwen3.5-122B-A10B,平均接受长度 5.01,接受率 80.2%);Qwen3.6-35B-A3B + DFlash 为 1.77–2.06×;最优提案长度 **N = 3–11**,随模型与数据集变化。价值在印出来的反面案例:**EAGLE-3 在 MATH500 上的最大实测值仍*低于*无推测基线——它可以更慢。** TL;DR 立即自我限定(结果"取决于模型家族、草稿检查点、工作负载与接受行为",全部数字"来自我们的测试环境")并坚持用"有代表性的工作负载和端到端测量"选配置。推测解码已快成为条件反射式建议;这篇的纪律是呈现它的范本。标准厂商基准限定同样适用:AMD 测 AMD。

## 带置信区间的量化：4-bit 站得住，1-bit 崩掉（09-09）

Quesma 的工程博客（8 月 26 日，9 月 8 日上趋势；已一手阅读）是少见的**带 Wilson 95% 置信区间、用 agentic 基准**
的量化研究：约 $3,000 租用 L40S/H100/H200（仅 Terminal-Bench 2.1 约 $2,308）、llama.cpp、Unsloth GGUF 量化版
Qwen3.8 27B 对比复现的 BF16 基线，测 GPQA Diamond / IFBench / Terminal-Bench 2.1（89 任务，98k 上下文）。
结果：**Q4_K_M（17 GB）**——"在这些基准上你察觉不到差别"——Terminal-Bench 追平 BF16，且装进 24 GB 卡；
**UD-Q2_K_XL（10.7 GB）**——"有点崩"但仍"是 Opus 4.7 或 Gemini 3.1 Pro 的水平"，每解一题多写约 25% token
（IFBench 到 2-bit 无变化）；**UD-IQ1_S/M（6.2 GB）**——"得分在随机猜测水平附近，最小的模型低于该阈值"，且在
`xhigh` 推理档耗尽 token 预算、返回空答案——更长的推理反而*伤害*坏掉的量化。这直接打脸 Unsloth
...
## 2026-09-10 04:03 — 端侧推理成为有定价的 SDK 层

- **Desert Ant Labs — 18 个任务专用端侧模型,一个 SDK,月活设备 10 万以下免费**(9 月 8 日发布;HN 326+ 分)。新欧洲实验室(_DETAIL 视频应用作者创立):一个 Swift/Kotlin/JS SDK 覆盖 18 个模型(12 个稳定)——Voz 在 iPhone 上 2 秒转写 10 分钟音频(宣称比 Whisper 快 4.7×;M3 Ultra 上 319× 实时,Apple SpeechAnalyzer 为 78×)、9MB Clear 音频增强、12MB Redact PII 遮蔽、284MB Clips 视频。发布帖自己做告诫:Redact PII 捕获 88.8%(GLiNER-PII 为 91.1%),全部基准自报,Clear 数字"三次取最优"。意义:端侧层有了产品形态——无 token、无登录、10 万设备以下免费,直击按 token 计费的云定价;"永不上传"的隐私论据在转写/遮蔽/分类这一应用层真实成立。本地 AI 的问题又移了一格:从"选哪个运行时"(llama.cpp/MLX)变成"选哪个 SDK"。

## 2026-09-10 20:03 —— 流式加载学派有了维护中的引擎；"能不能跑"变成一条命令；内存直接叠上芯片

- **colibri 再上趋势（+157/日，27.3k★）——08-28 的无 GPU 专家流式加载器如今以"维护中的引擎"而非"一次性绝活"立档。** 与 Kimi-K3-四 SSD 那次一次性构建不同：README 公开失败模式——推测解码是**实测净亏**（专家命中率 85% 附近 MTP −32%；DeepSeek drafter 默认关闭）、量化容器规则、依赖硬盘的 `O_DIRECT`——并逐项列出未证明的部分（放置策略、SSD 条带化、自动规划）。地板诚实：6× RTX 5090 上 5.8–6.8 tok/s，纯 CPU 热态 ~1.8 tok/s，25 GB 冷态 0.05–0.1。把失败与 tok/s 一起发布，才是数字可用的原因。
- **llmfit（AlexsJones，MIT，Rust，35.5k★，+247/日）**：检测 CPU/RAM/GPU/VRAM（CUDA、Apple Silicon、ROCm、oneAPI；多卡、MoE 感知），按四轴给目录模型打分——内存适配、预估速度、质量、上下文——带量化感知（GGUF、AWQ、GPTQ、EXL2）、TUI、Web 仪表盘与 REST 端点。诚实住在 `llmfit info` 里：速度与内存是基于内存带宽模型 + 社区 `bench --share` 提交的模型化估计；本地实测会替换估计。量化论坛里每天靠试错回答的"能不能跑"问题，变成一条命令，且估计与实测的界限始终可见。
