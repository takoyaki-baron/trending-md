...
2. **Agent security is the immediate attack surface — and every named class ends up enforced by
   ...
   - **09-09 04:03 — the record patch batch, and score-vs-exposure splits on both fronts (detail → [[security]]):** Microsoft's **974-CVE** Patch Tuesday with two same-day KEV zero-days — ZDI: the AI-discovery surge's "matching spike in active exploits hasn't yet materialized" (count ≠ incident rate); SAP OVERPASS (10.0 pre-auth) + S4GET (9.8); StyleSmuggler lands CVE-2026-75650 (10.0, KEV) with patch-plus-total-credential-rotation; LG OLEDs store-and-forward mic exfiltration — air-gapping stops transmission, not collection.
   ...
   - **09-09 20:03 — memory-only persistence; the zero-day count is a utilization rate (detail → [[security]]):** PoisonedRefresh (Sophos) prepends a PHP web shell to the *in-memory* copy of F5 BIG-IP APM's webtop scripts only — disk stays clean, defeating exactly the file-integrity checks run on load balancers (entry: CVE-2025-53521, KEV since March; no exploitation timeline, attribution unnamed); Chrome 153 ships 230 fixes with **CVE-2026-87491**, the year's seventh in-the-wild zero-day — while NVD rates it only Medium and Google withholds details: the in-the-wild status, not the score, sets the patch clock.
...
3. **Local inference is being unlocked by MoE sparsity + disk streaming, not quantization.**
   ...
   - **08-21→08-27 — tokenization, diffusion, MIT bases, lossless speculative decode; the KV cache optional; scan-boundary causality leaks get a vendor diagnostic (all detail → [[edge-inference]]):** RollTab, DiffusionGemma, Ling-3.0 MIT bases, Liquid DSpark, FreeToken, Daedalus-150M's clean cache ablation (6/18 blocks), Apple M6 2nm + M5 Ultra 512 GB/1.2 TB/s, QAH 4-bit-beats-bf16, CarWatch, Groq 3 LPX; "The Mask Is Not the Model" (arXiv 2608.22876) — Zamba2 + Nemotron-H leak at chunked-scan boundaries, productized as VIDRAFT **AX-RAY** — while Qwen3.8-Flash-Next / GLM-5.3-Flash go unaudited; ALPHABET (6,437 params, near-Bayes-oracle on a Gaussian control task).
   ...
   - **09-02 — the expert-streaming space fragments; quant surgery matures; the consumer blueprint lands (detail → [[edge-inference]]):** slotstream — the fifth-plus parallel MLX SSD-streaming impl, greedy decode byte-identical as a standing CI test; Tiel-Coder-35B (community surgery, n=25 honesty; Ornith's MTP head shipped random-init until Aug 23 — check the checkpoints); M4 Pro Mac mini + Qwen3.6-35B-A3B-OptiQ-4bit on oMLX at 325/34 tok/s — "active parameters × quantization is what fits in RAM."
