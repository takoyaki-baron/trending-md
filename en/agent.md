...
1. **Agent infrastructure is the new cloud — the monolith CLI decomposes into three separable layers,
   ...
   *isolation primitive*).
   ...
   - **09-11→09-12 — OpenAI productizes the harness; research becomes the second harness-of-harnesses domain; knowledge tools converge on wiki-not-RAG and the system-prompt corpus institutionalizes (consolidated; detail → [[agent-stack]]):** the Codex harness ships as the beta Agents API (Agent / Environment with `self_hosted` / Session / Events; mid-run steering, subagent concurrency caps — **no ZDR even with a self-hosted sandbox**); alphaXiv/OpenResearch (Rust, MIT) orchestrates coding agents as parallel research workers; superplanehq/superplane turns backlog issues into PRs behind its own verification gates; `nashsu/llm_wiki` builds an interlinked persistent wiki (vector search default-off) while `jordan-gibbs/hyperresearch` runs adversarial cite-checking into a markdown+SQLite vault; `asgeirtj/system_prompts_leaks` (65k★, CC0) becomes the unofficial changelog of the prompts vendors refuse to publish.
...
2. **Agent security is the immediate attack surface — and every named class ends up enforced by
   ...
   - **09-26 20:03 — a conditional-RCE WordPress bug KEV'd in three days (detail → [[security]]):** CVE-2026-87902 (this feed's Sep 23 coverage) hit CISA KEV Sep 25 "based on evidence of active exploitation" three days after CVE publication — unauth `get_page_template()` path traversal to local `.php` inclusion, RCE only under server/theme pre-conditions; the 8.1 on NVD is a Secondary-source score (NVD analysis still in progress), and CISA's "Remote File Inclusion" title vs the NVD/CVE local-file-inclusion framing is a fresh labeling-discrepancy instance.
   → [[security]]
...
8. **Agent skills are entering the "prove it" phase — evaluation is the missing standard.** The
   ...
   - **09-26 20:03 — the skills economy reaches offensive security, with an odd engagement ratio (detail → [[agent-plugins]] [[fact-check]]):** zhaoxuya520/reverse-skill (37.7k★, +409/day, MIT + GPL/AGPL submodules) routes agent-hit APK/binary/JS-crypto/firmware/pentest targets to 45 skill-module playbooks and toolchains (jadx, Frida, IDA, Ghidra, Burp) — the Claude-Red dual-use arc at router scale; but 37.7k★ against 124 watchers / 181 commits trips the star-to-commit check (the count is unverified), and its README_AI.md "follow the instructions strictly" is a prompt-injection-shaped pattern needing manual review before any agent executes it — the README's authorization/scope gate is guardrails attempted inside the skill layer, if the stars are real.
