...
1. **Agent infrastructure is the new cloud — the monolith CLI decomposes into three separable layers,
   ...
   *isolation primitive*).
   - **08-16→09-09 — the stack decomposes by layer (plugin graph / state kernel / worktree isolation); memory gets an envelope spec, then a Bilevel Coordinated Reflection impossibility result for transcript-only gates; provenance gets a Xanadu bet; the frontier-lab sandbox is mapped first-hand (undocumented Antspace); Alibaba ships its internal reviewer with an annotated trade-off bench; the viral-skill-pack caution ratio recurs (OpenMontage 58k★ vs 449 commits); ruflo's Agent Federation, LatentPress memory tokens, aipoch/open-science checksummed provenance; design and math state their evidence contracts; orchestration refuses self-grading (Ordewell VerdictEngine); fleets go multi-machine (herdr); the team-config layer arrives (Tencent teamai-cli) (consolidated; all detail → [[agent-stack]] [[frontier-models]]).**
...
2. **Agent security is the immediate attack surface — and every named class ends up enforced by
   ...
   - **08-16→09-16 — two consolidated sweeps fill in the sixteen shapes (time-to-exploit goes negative, patch-then-reverse-engineer, third-party ASM scanning leaks, AI-serving proxy, loopback is not a trust boundary), then 2013-era mistakes at production scale; a same-day KEV batch (GitLab 10.0 KEV'd <24h post-patch, JFrog Artifactory confirmed chained in the wild, ScreenConnect NVD-9.9), a primary-source correction, a second independent grid for the agentic-offense watch; the mailing list and the test server as initial-access surfaces (Trezor × Brevo, ~347k phished); a hardware keepalive bypassing Mullvad's VPN kill switch (consolidated; all detail → [[security]]).**
...
8. **Agent skills are entering the "prove it" phase — evaluation is the missing standard.** The
   ...
   - **08-18→09-05 — assertion-only repos; the measurement machinery fills in and prints its first failure baseline (FrontierChallenge: 75.5% of non-passing Claude Code trajectories claimed completion); the Vals SkillsBench leaderboard lands but the famous skills ship no numbers; `anthropics/skills` trends #5 at ~512★/day with no release — the examples repo out-velocities product launches (consolidated; all detail → [[agent-plugins]]).**
