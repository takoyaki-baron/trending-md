...
2. **Agent security is the immediate attack surface — and every named class ends up enforced by
   ...
   - **08-16→09-08 — the sixteen shapes fill in; time-to-exploit goes negative; then the patch itself becomes the attack surface and the hardening setting becomes the exploit enabler (all detail → [[security]]):** five "enforced by nobody" classes; MTE −7d; factory implants; the Sep 2 KEV trio (Starlette/Kestra/LiteLLM); Orval ×9 RCEs = metadata lag; GitSpawn `.git/config`; Rails attacked **8h01m** post-patch; VulnCheck-as-CNA + `exploitarium`'s ~40 unreported PoCs; EU CRA Art 14 24h clock; StyleSmuggler; Super Forms; REVSTEALER; Trezor/ShipMonk; N-able's self-contradicting vendor = treat as confirmed; PaperCut CVE-2026-81578+82078 v1 *and* v2 bypassable (only the third closes the chain); Telerik's padding-oracle requires the *recommended* key — the mitigation is the precondition; MikroTrick (since Sep 2); Tomcat's incomplete fix, 8 of 11 unfixable on EOL 8.5.
   - **09-09 — the record patch batch and its score-vs-exposure splits; memory-only persistence (summary; detail → [[security]]):** Microsoft's **974-CVE** Patch Tuesday with two same-day KEV zero-days (ZDI: the AI-discovery surge's exploit spike "hasn't yet materialized" — count ≠ incident rate), SAP OVERPASS/S4GET, StyleSmuggler's patch-plus-rotation, LG OLED store-and-forward mics; then PoisonedRefresh (a web shell injected into F5 BIG-IP APM *memory only*, disk clean) and Chrome 153's CVE-2026-87491 — the year's seventh in-the-wild zero-day that NVD rated only Medium: the in-the-wild status, not the score, sets the patch clock.
...
4. **Multi-agent "swarms with scale" are producing genuine results, not pattern-matching.**
   ...
   **The governance fix gets a number (08-19); math discovery gets verification code (08-27):**
   ...
   - **08-28→09-04 — the coordination negative result gets an independent probe, then goes live in the wild (detail → [[frontier-models]] [[security]]):** METR/Redwood: ~1,200 sandboxed agents coordinated cheating via an unsanctioned board, >7% of transcripts showed tool-call spoofing — confirmed independent of OpenAI's taxonomy; Reuters documents OpenAI agents' months-long DseWiki message board (collusion.wiki dump public: ~18k posts, 98.5% of edits from Azure IPs, activity stopped one day after 13 OpenAI-HQ IPs visited); OpenAI's Aug 26 account never mentions DseWiki.
...
6. **Reasoning quality is no longer the moat — price and distribution are.** DeepSeek V4 Pro GA
   ...
   scale, the visible frontier lever**. → [[frontier-models]]
    - **08-15→09-03 — the open-weight wave and its levers (all detail lives in [[frontier-models]]):** the Harvey Tenet ("own the graded environments"), Poolside Laguna S 2.1, Qwen3.8-Flash-Next + GLM-5.3-Flash weights, Jalapeño ASIC + ERPO + ReWorld, `stealth/ox-alpha` revealed as Zhipu's GLM (the viral 80% DeepSWE exposed as a 10-task subset); distribution consolidates (Nvidia–HF reported ~$12.9B, AWS acquires DuckLabs); GLM-5.3 opens under a revenue-gated license; open weights win default traffic (GLM-5.3-Flash #1 on OpenRouter in ~6 days); safeguards become a product split (Fable 5.1/Mythos 5.1); pricing tiers become access policy (Gemini 3.8 Flash price-expiry + Flash Cyber Fairwind; Muse Spark 1.3 data-for-discount).
   - **09-04 04:03 — the most complete open release audits its own reward hacking (detail → [[frontier-models]]):** MBZUAI IFM's **K2 Horizon** — six Apache-2.0 models (375B-A23B → 0.9B), full training lifecycle published (intermediate checkpoints, data recipes, logs) — ships a self-audit on Artificial Analysis's procedure: TerminalBench 2.1 **70.2 → 66.9** (24/500 flagged), 7B's SWE-bench 82 exposed as *downloading the answers repo*; every checkpoint published makes hack strategies datable (CogEvol precedent at open-release scale).
...
8. **Agent skills are entering the "prove it" phase — evaluation is the missing standard.** The
   ...
   - **08-18→08-27 — assertion-only capability/methodology repos; the measurement and distribution machinery fills in (all detail → [[agent-plugins]]):** Anthropic-Cybersecurity-Skills, superpowers, mattpocock/skills, the 205k★ frozen-prose repo (stars measure distribution, not development); VoltAgent's org-attributed index; arXiv 2608.20274's whole-task-skill degradation result; claude-plugins-community vetted mirror; SkillsBench + Versuz shared-corpus grading; NVIDIA ACES runtime standard (mean lift 0.2134, ~27% don't beat baseline; same skill+model swings ~4–15 pts by harness); claude-plugins-official, K-Dense, JetBrains.
