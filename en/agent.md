---
title: Learnt Agent
last_processed: 2026-09-21T12:32:00+08:00
---
...
1. **Agent infrastructure is the new cloud — the monolith CLI decomposes into three separable layers,
   ...
   *isolation primitive*).
   ...
   - **09-16 — orchestration refuses self-grading; the agent extends its own UI, gets a device farm and a governed test world (consolidated; all detail → [[agent-stack]]):** Ordewell's VerdictEngine ("the model is never the tie-breaker"); greentfrapp/panel agent-built panes; vphone-cli's iPhone-IPSW VM device farm with an MCP server; Datamimic CE vs the agent-fabricated-fixture failure.
...
2. **Agent security is the immediate attack surface — and every named class ends up enforced by
   ...
   - **09-16 — 2013-era mistakes at production scale; the update channel itself is the weapon (consolidated; all detail → [[security]]):** the Baseten PAT-in-Docker-layer, vCenter ransomware two months post-patch, marimo's 8-second chain, the trojanized-then-re-trojanized WP extension, Delinea vault SAML impersonation, Japan Digital Agency's known-flaw breach, Cloudflare's audit-skill as the defensive mirror.
...
6. **Reasoning quality is no longer the moat — price and distribution are.** DeepSeek V4 Pro GA
   ...
   - **09-16→09-17 act — Jev watch consolidated (detail → [[frontier-models]] [[system1-decision]]):** docs + self-serve API keys live (`jev-latest`), pricing still unpublished ($0.042/MTok blog-only), $40M from stealth (DCVC); the thread response is recreations, not rebuttals or measurements — still zero TypeSafe comments; OpenJev's browser replication (84.5% vs hosted 88.3%) is the first community reference point.
