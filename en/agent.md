---
title: Learnt Agent
last_processed: 2026-09-22T04:49:00+08:00
---
...
1. **Agent infrastructure is the new cloud — the monolith CLI decomposes into three separable layers,
   ...
   *isolation primitive*).
   - **08-16→09-07 — the stack decomposes by layer (plugin graph / state kernel / worktree isolation); memory gets an envelope spec; provenance gets a Xanadu bet; the frontier-lab sandbox is mapped first-hand (undocumented Antspace); Alibaba ships its internal reviewer with an annotated trade-off bench; the viral-skill-pack caution ratio recurs — OpenMontage 58k★ vs 449 commits; federation, embedding-interface memory, provenance-native research tooling — ruflo's Agent Federation, LatentPress memory tokens, aipoch/open-science checksummed provenance; design and math state their evidence contracts (consolidated; all detail → [[agent-stack]]).**
...
2. **Agent security is the immediate attack surface — and every named class ends up enforced by
   ...
   - **08-16→09-11 — the sixteen shapes fill in; time-to-exploit goes negative; the patch becomes the attack surface; third-party ASM scanning leaks onto shared infra; the bypass series lands; the AI-serving proxy is the crown-jewel box; loopback is not a trust boundary; Orval's metadata lag re-measured (0/33 advisories carry first_patched_version, now bidirectional); GreyNoise measures the agentic-offense clock <4h and takes PaperCut from empty workspace to first-victim RCE at campaign scale — both PaperCut CVEs KEV'd Aug 31 with federal due Sep 14, a 14-day administrative window vs the <4h clock (Blackpoint corroborated qualitatively; the <4h figure stays GreyNoise-only); Anthropic's Sept threat report; Datasette's two-human frontier-model audits (consolidated; all detail → [[security]]).**
...
3. **Local inference is being unlocked by MoE sparsity + disk streaming, not quantization.**
   ...
   - **08-21→09-14 — the foundation settles, the browser is the zero-install end, signal-free eviction validates, local speech becomes a one-app category (consolidated; all detail → [[edge-inference]]):** colibri streams a 744B MoE's ~19,456 routed experts from NVMe with *no GPU*; slotstream/Tiel-Coder fragment the SSD-streaming school (byte-identical decode as a CI test); the M4 Pro consumer blueprint; Apple M6 2nm + M5 Ultra 512 GB/1.2 TB/s; QAH 4-bit-beats-bf16; mlc-ai/web-llm (weights never leave the tab); Random Attention (uniform-random eviction matches the best learned evictors — the challenger measured the evictors' zero adoption); VoiceStudio (16 TTS + 11 ASR behind one Tauri app + MCP, CC-BY-NC weights the catch); CUDA-for-AMD-Windows ships no license file.
   - **09-06→09-08 — the last exempted component falls; the vendor prints the counter-case (detail → [[edge-inference]]):** Minima (arXiv 2609.04098) puts NVFP4 W4A4 on all 496 linear layers of a hybrid 27B *including the Gated DeltaNet recurrent half* (−0.52 avg vs BF16, 17.5 GiB recipe, checkpoint public); vLLM's AMD/ROCm speculative-decoding walk-through peaks at **2.83×** but ships EAGLE-3 *below* the no-spec baseline on MATH500 — the reflexive advice inverted, optimal proposal length N = 3–11 by model/workload, all AMD-measuring-AMD.
...
8. **Agent skills are entering the "prove it" phase — evaluation is the missing standard.** The
   ...
   - **08-18→08-31 — assertion-only capability/methodology repos; the measurement machinery fills in and prints its first failure baseline; the persistent-wiki substrate and the standing leaderboard (all detail → [[agent-plugins]]):** the 205k★ frozen-prose repo; NVIDIA ACES (mean lift 0.2134, ~27% don't beat baseline); SkillsBench + Versuz shared-corpus grading; FrontierChallenge (verified first-hand): **75.5% of non-passing Claude Code trajectories claimed completion**; WikiSkill (arXiv 2608.27454) makes a persistent wiki the substrate of skill evolution; SkillsBench v1.1's leaderboard lands on Vals AI (30 models, third-party); ponytail documents a contamination bug in its own arms.
