...
2. **Agent security is the immediate attack surface — and every named class ends up enforced by
   ...
   - **08-16→09-07 — the sixteen shapes fill in; the patch window goes negative; both exploitation clocks measured end-to-end; proof-of-life failures on both sides of the ledger (all detail → [[security]]):** five "enforced by nobody" classes; MTE −7d; factory implants; the Sep 2 KEV trio (Starlette/Kestra/LiteLLM); Orval ×9 RCEs = metadata lag; GitSpawn `.git/config`; Rails attacked **8h01m** post-patch; VulnCheck-as-CNA + `exploitarium`'s ~40 unreported PoCs; EU CRA Art 14 24h clock; StyleSmuggler; Super Forms; REVSTEALER; Trezor/ShipMonk; and N-able N-central CVE-2026-86218 — when the vendor's own release notes and incident notice contradict each other, treat exploitation as confirmed until proven otherwise.
   - **09-08 04:03 — the patch itself becomes the attack surface; the hardening setting becomes the exploit enabler (detail → [[security]]):** PaperCut CVE-2026-81578+82078 — patches v1 *and* v2 bypassable, only the third closes the chain; Telerik's padding-oracle-to-RCE requires the *recommended* `ConfigurationEncryptionKey` — the mitigation is the precondition; MikroTrick SSH chain exploited since Sep 2; Tomcat's 11-CVE drop includes an *incomplete fix* for CVE-2026-32990, 8 of 11 unfixable on EOL 8.5.
...
3. **Local inference is being unlocked by MoE sparsity + disk streaming, not quantization.**
   ...
   - **08-21→08-26 — tokenization, diffusion, MIT bases, lossless speculative decode; the KV cache optional; the hardware half of the fit-to-budget turn (detail → [[edge-inference]]):** RollTab, DiffusionGemma, Ling-3.0 MIT bases, Liquid DSpark, FreeToken bandwidth-adaptive whole-machine serving; Daedalus-150M's clean cache ablation (6/18 blocks); Apple M6 2nm + M5 Ultra 512 GB / 1.2 TB/s; QAH 4-bit-beats-bf16; CarWatch; Groq 3 LPX.
