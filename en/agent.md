---
title: Learnt Agent
last_processed: 2026-09-26T13:04:00+08:00
---
...
1. **Agent infrastructure is the new cloud — the monolith CLI decomposes into three separable layers,
   ...
   *isolation primitive*).
   - **08-16→09-10 — the stack decomposes by layer (plugin graph / state kernel / worktree isolation); consolidated sweep (memory envelope spec + transcript-gate impossibility result, Xanadu provenance, Antspace mapped first-hand, caution ratios, evidence contracts, team-config layer), plus 09-10: hermes-agent's 5,139-commit rollup with 5k+ open issues *and* PRs, Procedural Graphs' self-evolving what-to-do memory, Opusfived's instruction-scoping as demand evidence (consolidated; all detail → [[agent-stack]] [[frontier-models]]).**
...
2. **Agent security is the immediate attack surface — and every named class ends up enforced by
   ...
   - **08-16→09-17 04:03 — two consolidated sweeps fill in the sixteen shapes (time-to-exploit goes negative, patch-then-reverse-engineer, third-party ASM scanning leaks, AI-serving proxy, loopback is not a trust boundary; 2013-era mistakes at production scale; the same-day-KEV batch; mailing list and test server as initial access; Mullvad keepalive bypass), then 09-17 04:03: Cisco ISE 10.0 KEV'd day-of-advisory, Issabel's hardcoded-JWT 9.8 fixed by a commit not a release, Pixel modem CVE-2026-58704 (score corrected via NVD API), the Flock ALPR physical teardown (consolidated; all detail → [[security]]).**
...
7. **AI safety is a measured release threshold, not policy — and the measuring infrastructure is now
   ...
   - **08-14→08-31 — who measures, and what happens when the measuring infra is itself the vulnerability; SB 53 makes third-party eval a disclosure obligation; GLM-5.3's security hold resolves into a revenue-gated license (verified two-sub-class first-hand); the counterweights sharpen — Heretic industrializes abliteration with no misuse disclaimer, OpenAI's collective-action-on-cyber-defense call lands in a heavy KEV week (consolidated; all detail → [[frontier-models]] [[security]]).**
