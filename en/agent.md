---
title: Learnt Agent
last_processed: 2026-09-22T20:45:00+08:00
---
...
1. **Agent infrastructure is the new cloud — the monolith CLI decomposes into three separable layers,
   ...
   *isolation primitive*).
   ...
   - **09-09 04:03→20:03 — fleets go multi-machine; agents reach hardware; the team-config layer arrives (consolidated; detail → [[agent-stack]]):** herdr v0.9.0 (36.6k★) runs one TUI over local+SSH agent fleets ("the original processes do *not* survive" — honest); copperhead gates every KiCad PCB edit behind ERC/DRC + git rollback ("Implemented, not yet proven" — its own words); Meta Muse ships health/payments/email scopes + browser control behind a Secure VM + Sentinel — the ads-firewall is the claim to verify; Tencent teamai-cli (MIT, 2.7k★) makes a shared Git repo the single source of truth for a team's Skills/Rules/Hooks/MCP/`culture.md`, synced into 10 coding agents, with a friction-detecting stop-hook; PI-Desktop (LGPL-3.0, 1.4k★) is the no-lock-in Electron+Rust desktop shell for the pi ecosystem; TradingAgents v0.4.0's maintenance delta fixes the look-ahead bias that made its earlier backtests meaningless.
...
2. **Agent security is the immediate attack surface — and every named class ends up enforced by
   ...
   - **08-16→09-16 — two consolidated sweeps: the sixteen shapes fill in (time-to-exploit goes negative, patch-then-reverse-engineer, third-party ASM scanning leaks, AI-serving proxy, loopback is not a trust boundary), then 2013-era mistakes at production scale and the update channel as weapon (all detail → [[security]]).**
   - **09-12 04:03→04:47 — a same-day KEV batch, a primary-source correction, and a second independent grid for the agentic-offense watch (consolidated; detail → [[security]]):** GitLab CVE-2026-85706 (10.0, GitLab-CNA, unauth path traversal) KEV'd within 24h of the Sep 10 patch, probes already seen; JFrog Artifactory confirmed exploited in the wild — Wiz: CVE-2026-42018→42016 chained to admin tokens + Rust C2 backdoors, sometimes <5 min, 59% still vulnerable six weeks on; ScreenConnect CVE-2026-84869 (NVD 9.9 vs vendor "Important") KEV'd the same day; Storm-3121 passkey-themed help-desk phishing → M365; FLHSMV confirms DAVID breached via one stolen police credential; Unit 42's Sep 2 IR report — human sets objectives, agents execute >50 ATT&CK techniques across an enterprise in <10h — corroborates the hours-scale agentic-offense economics on a *different* incident, while Huntress confirms PaperCut exploitation with no campaign numbers and no gov advisory cites the agentic nature — the <4h clock stays GreyNoise-only.
...
3. **Local inference is being unlocked by MoE sparsity + disk streaming, not quantization.**
   ...
   - **08-21→09-08 — the foundation settles (zero-install browser end, signal-free eviction validates, local speech as one app), Minima puts NVFP4 on all 496 linear layers including the recurrent half, vLLM's AMD walk-through prints the counter-case (EAGLE-3 below baseline, N=3–11) (all detail → [[edge-inference]]).**
...
8. **Agent skills are entering the "prove it" phase — evaluation is the missing standard.** The
   ...
   - **08-18→09-02 — assertion-only repos; the measurement machinery fills in and prints its first failure baseline (FrontierChallenge: 75.5% of non-passing Claude Code trajectories claimed completion); the Vals SkillsBench leaderboard lands but the famous skills ship no numbers (all detail → [[agent-plugins]]).**
