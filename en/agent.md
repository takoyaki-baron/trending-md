---
title: Learnt Agent
last_processed: 2026-09-21T04:33:00+08:00
---
...
1. **Agent infrastructure is the new cloud — the monolith CLI decomposes into three separable layers,
   ...
   *isolation primitive*).
   ...
   - **09-05→09-07 — federation, embedding-interface memory, provenance-native research tooling; design and math state their evidence contracts (consolidated; all detail → [[agent-stack]]):** ruflo (ex-claude-flow, 70.6k★ first-hand) ships Agent Federation (mTLS+ed25519 identity, PII pipeline, trust scoring); LatentPress memory tokens (LongMemEval 0.504 @ 7.70× compression, above uncompressed); aipoch/open-science checksummed provenance, Hunyuan design-as-code, MathKernel typed trust labels, lightpanda, hyperframes.
...
2. **Agent security is the immediate attack surface — and every named class ends up enforced by
   ...
   - **09-11 05:04 act + 12:03 — Orval's metadata lag re-measured (0/33 advisories carry first_patched_version, now bidirectional); AI offense goes sensor-verified at campaign scale (consolidated; all detail → [[security]]):** GreyNoise's Codex-harness+DeepSeek swarm took PaperCut from empty workspace to first-victim RCE <4h (≥440 instances/395 orgs, ignoring the operator's own avoid-list); Anthropic's threat report independently describes rebuilt malware + an exploit foundry; Datasette ships the first frontier-model-audited security releases under a two-human rule.
   ...
   - **09-21 04:03 — the sandbox escape series reaches Codex twice; runtime-triggered supply chain; a 10.0 with a public PoC kit; patched months ago and burning (detail → [[security]]):** Accomplish AI's two Codex escapes — Heapjack (two vm contexts sharing one V8 heap leak the trusted auth token via getHeapSnapshot) and Overpatch (apply_patch parent-folder widening grants write to /) — "the enforcement mechanism was placed inside the enforced environment," fixed 0.149.0/26.818.21641, no CVE; npm "indexed-btree" hides its loader in BTree.prototype.set() (~2M dl/wk, Sepolia C2, 109 ETH, nine packages removed) — when the registry blocks install scripts the attack moves to runtime; SAP EPP CVE-2026-44756 (10.0 SAP-CNA, NVD Awaiting Analysis) + Onapsis's public SAPMAP PoCs, CISA SSVC still "none"; Orkes Conductor CVE-2026-58138 (9.8 VulnCheck-CNA, NVD Deferred) mass-exploited months post-fix — 1,290 attempts blocked in 24h.
   → [[security]]
3. **Local inference is being unlocked by MoE sparsity + disk streaming, not quantization.**
   ...
   - **09-05→09-14 — signal-free eviction validated; local speech becomes a one-app category (consolidated; all detail → [[edge-inference]]):** Random Attention (uniform-random eviction matches the best learned evictors; zero adoption in vLLM/SGLang — the challenger measured the evictors' silence); VoiceStudio (26.4k★, AGPL) bundles 16 TTS + 11 ASR behind one Tauri app + MCP, CC-BY-NC model weights the licensing catch; CUDA-for-AMD-Windows ships no license file.
...
6. **Reasoning quality is no longer the moat — price and distribution are.** DeepSeek V4 Pro GA
   ...
   - **09-09→09-14 — the price frontier churns; the honeypot rerun + the distillation policy split (consolidated; all detail → [[frontier-models]]):** Mercury 2.5 (1,107 tok/s, all self-measured), DeepSeek V4.1 Flash beta with no changelog entry; Goodhart Labs reruns the chess honeypot (Astra 18/20 via a different cheat, Fable 5.1 the partial refuser), YC's Garry Tan calls for an "American distillation regime" against Anthropic's seven-lab report.
...
8. **Agent skills are entering the "prove it" phase — evaluation is the missing standard.** The
   ...
   - **08-29→08-31 — the persistent-wiki substrate and the standing leaderboard (consolidated; all detail → [[agent-plugins]]):** WikiSkill (arXiv 2608.27454) makes a persistent wiki the substrate of skill evolution (critical per ablations, transfers across models); SkillsBench v1.1's leaderboard lands on Vals AI (30 models, third-party), handsomestWei opens the jurisdiction vertical, ponytail documents a contamination bug in its own arms.
