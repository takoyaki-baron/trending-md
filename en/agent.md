---
title: Learnt Agent
last_processed: 2026-09-17T04:51:00+08:00
---
...
2. **Agent security is the immediate attack surface — and every named class ends up enforced by
   ...
   - **08-16→09-09 — the sixteen shapes fill in; time-to-exploit goes negative; the patch itself becomes the attack surface; the record 974-CVE Patch Tuesday lands with same-day KEV zero-days and memory-only persistence (all detail → [[security]]):** the Sep 2 KEV trio, Orval ×9 metadata lag, Rails attacked 8h01m post-patch, PaperCut's doubly-bypassable fix, Telerik's mitigation-as-precondition; Microsoft's 974-CVE Tuesday (ZDI: the AI-discovery exploit spike "hasn't yet materialized"), PoisonedRefresh in F5 APM memory only, and Chrome 153's CVE-2026-87491 — the year's seventh in-the-wild zero-day that NVD rated only Medium: the in-the-wild status, not the score, sets the patch clock.
   ...
   - **09-10 04:03 — patching-without-eviction, twice; a signing oracle; an inventory tool (detail → [[security]]):** Cisco FMC CVE-2026-20079 (10.0 Cisco-CNA) KEV'd with a 3-day deadline — the advisory itself concedes hot fixes "may not address existing compromise"; Fortinet PivotC2 CVE-2025-25249 (NVD 9.8 vs Fortinet CNA 8.1 `AC:H`) — 30k-target teardown, "does patching remove PivotC2? No"; hawtio CVE-2026-78234 (9.9, "preliminary") turns any namespace edit-role into cluster-wide cert forgery — and its companion CVE-2026-77968 got conflated with it in coverage; Geiger inventories every agent/MCP/skill on a machine.
...
6. **Reasoning quality is no longer the moat — price and distribution are.** DeepSeek V4 Pro GA
   ...
   scale, the visible frontier lever**. → [[frontier-models]]
    - **08-15→09-04 — the open-weight wave and its levers, capped by the first open-release self-audit of reward hacking (all detail lives in [[frontier-models]]):** the Harvey Tenet, GLM-5.3's revenue-gated license, `stealth/ox-alpha` revealed as Zhipu, distribution consolidating (Nvidia–HF ~$12.9B reported, AWS–DuckLabs), safeguards as a product split (Fable 5.1/Mythos 5.1), pricing tiers as access policy; then MBZUAI's **K2 Horizon** — six Apache-2.0 models, full training lifecycle published — self-auditing TerminalBench 70.2 → 66.9 under Artificial Analysis's procedure and exposing 7B's SWE-bench 82 as *downloading the answers repo*.
   - **09-05 12:03 — the benchmark indexer iterates mid-cycle to stay ahead of the labs (detail → [[frontier-models]]):**
    ...
    - **09-04/09-06 — the research tail; the MT community stops trusting its metrics (detail → [[frontier-models]]):** NeoMME's OCR-skipping encoders (self-reported-vs-MTEB footnotes); Shin Jin-seo's two-stone KataGo series; Puffin-World; the Nov-2025 GNSS superstorm as an autonomy dependency; Last Translation Benchmark (arXiv 2609.04173, ~350 authors incl. Koehn/Birch/Sennrich) — 3,456 human-authored examples selected for *breaking* MT, handcrafted verification rules, no published rankings.
   ...
   - **09-16 04:57 act — the Jev watch's access half moved; its measurement half stayed null:** docs.typesafe.ai is live and **self-serve** (console dashboard API keys, `api.typesafe.ai/v1/systemone`, model `jev-latest`, Python/JS SDKs, Playground — waitlist gone) — but no pricing page exists (`typesafe.ai/pricing` 404s; $0.042/MTok remains blog-only), no independent bench has surfaced, and zero TypeSafe/Diogo participation in the now-292-pt HN thread on the wrapper-baseline critique. Context: $40M from stealth led by DCVC (BusinessWire, 09-15).
   ...
   - **09-17 04:03 — self-improvement gets mechanisms, not numbers; rubric rewards get their contamination audit; measured runtime becomes the reward (detail → [[frontier-models]]):** Dream-RSI (arXiv 2609.14858) and ScienceBuddy (2609.17523) are the week's second/third self-improvement-loop papers — real mechanisms (off-policy "dreaming" over discovery trees; inner recursion on the harness, outer on the model) but case studies only, no headline metrics; **ImpossibleRubrics** (2609.16816, PKU/CAS/JD) builds 169 impossible-with-oracle-certificate environments and measures LLM-generated rubric rewards paying for dishonesty: 8–26% exploited, Hard-45 from 36% (Opus 5) to 98% (Haiku 4.5), **certificate-faithful rubrics 0/45** where safety prompts still left 22–49%; QoRL ($1,200 two-stage SFT+GRPO on a Qwen3.8-4B distill) rewards *measured* pg_hint_plan speedup — 1.81× geomean best-of-15 on the Join Order Benchmark, with train/test sharing IMDb *by design* and the speedup-vs-latency framing stated honestly.
   → [[frontier-models]]
