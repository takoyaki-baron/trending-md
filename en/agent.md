...
1. **Agent infrastructure is the new cloud — the monolith CLI decomposes into three separable layers,
   ...
   *isolation primitive*).
   ...
   - **08-23 13:03→21:04 — memory gets its envelope spec; the stack recombines in one repo (detail → [[agent-stack]]):** no MCP SEP touches memory semantics — the W3C AI Agent Memory Interop CG (launched 2026-06-03, referencing IETF `draft-saihm-memory-protocol`) still declines authorship/confidence/provenance — envelope first, semantic record later; Hermes Agent (MIT, **234,615★, 34,925 open issues** — backlog, not stars, is the maintenance signal) rebundles skills+memory+6 gateways+7 terminal backends; Buzz (29.9k★) makes every event a signed **Nostr** event — provenance from the format.
...
2. **Agent security is the immediate attack surface — and every named class ends up enforced by
   ...
   **sixteen recurring shapes**, each with a canonical instance — the full shape→instance map lives in
   ...
   - **08-27 04:15→08-28 12:15 — agents find human-rare chains, then the KEV season and turnkey PoCs (detail → [[security]]):** Wordfence **Argus** chained six flaws → Avada unauth RCE (CVE-2026-18431, 9.8), WP HackerOne 20–30/month → 450 in July; ownCloud CVE-2023-49105 KEV'd, Chainlit CVE-2026-45018 (second MCP-stdio RCE), Gitea CVE-2026-60004 in-the-wild, PaperCut zero-day → SYSTEM, Redis QVD-2026-58458 (public PoC), turnkey unauth PoCs for TranslatePress/Tutor LMS/Elementor Pro, Xiiaozet ICS 2× 9.8.
