---
title: Learnt Agent
last_processed: 2026-09-25T21:02:00+08:00
---
...
1. **Agent infrastructure is the new cloud — the monolith CLI decomposes into three separable layers,
   ...
   *isolation primitive*).
   - **08-16→09-07 — the stack decomposes by layer (plugin graph / state kernel / worktree isolation); memory gets an envelope spec; provenance gets a Xanadu bet; the frontier-lab sandbox is mapped first-hand (undocumented Antspace); Alibaba ships its internal reviewer with an annotated trade-off bench; the viral-skill-pack caution ratio recurs — OpenMontage 58k★ vs 449 commits; federation, embedding-interface memory, provenance-native research tooling — ruflo's Agent Federation, LatentPress memory tokens, aipoch/open-science checksummed provenance; design and math state their evidence contracts; 09-16: orchestration refuses self-grading (Ordewell VerdictEngine), agent-built panes, virtual-iPhone device farm, governed test data (consolidated; all detail → [[agent-stack]]).**
   - **09-08→09-09 — memory gates get an impossibility result; fleets go multi-machine; the team-config layer arrives (consolidated; detail → [[agent-stack]] [[frontier-models]]):** Bilevel Coordinated Reflection (arXiv 2609.02750) proves **no gate that observes only the generated transcript can improve uniformly over text-indistinguishable environments** — memory acceptance needs a grounded gate; Engrim shares one SQLite memory file across Claude Code/Cursor/Codex with per-record `origin_agent` provenance; DeerFlow 2.0 (81.8k★) ships controlled sandbox-egress approvals; herdr v0.9.0 (36.6k★) runs one TUI over local+SSH agent fleets; copperhead gates every KiCad PCB edit behind ERC/DRC + git rollback; Tencent teamai-cli (MIT) makes a shared Git repo the single source of truth for a team's Skills/Rules/Hooks/MCP/`culture.md`, synced into 10 coding agents; TradingAgents v0.4.0 fixes the look-ahead bias that made its earlier backtests meaningless.
