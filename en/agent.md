...
1. **Agent infrastructure is the new cloud — the monolith CLI decomposes into three separable layers,
   ...
   *isolation primitive*).
   - **08-16→09-14 — the stack decomposes by layer (plugin graph / state kernel / worktree isolation); memory gets an envelope spec; provenance gets a Xanadu bet; the frontier-lab sandbox is mapped first-hand (undocumented Antspace); Alibaba ships its internal reviewer with an annotated trade-off bench; the viral-skill-pack caution ratio recurs — OpenMontage 58k★ vs 449 commits (consolidated; all detail → [[agent-stack]]).**
...
2. **Agent security is the immediate attack surface — and every named class ends up enforced by
   ...
   - **08-16→09-11 — the sixteen shapes fill in; time-to-exploit goes negative; the patch becomes the attack surface; third-party ASM scanning leaks onto shared infra; the bypass series lands (three Defender bypasses, ~9,000 WatchGuards still feeding ransomware); the AI-serving proxy is the crown-jewel box (Wiz: 9.6% of exposed LiteLLM accept the example admin key); loopback is not a trust boundary (DeepSeek Harness CVE-2026-82533); GreyNoise measures the agentic-offense clock <4h (consolidated; all detail → [[security]]).**
   ...
   - **09-20 04:35 — the eval sandbox breaks for a fourth lab; criminals breach the criminals; hand-rolled JS sandboxes still yield root (detail → [[security]]):** Gemini reached three real companies from a broken Irregular CTF harness (same bug already reported to all relevant labs in late July; Google's first acknowledgment of autonomous third-party access — the harness, not the model, was the vulnerability; "the model stopped" is Google's own characterization); ShinyHunters breaches Clop's leak site (claims the onion keys; the Grav CMS vector is the attackers' unverified claim); OpenPanel CVE-2026-93985 (9.9 VulnCheck-CNA, no patch — `['constructor']['constructor']` past an AST allowlist into `new Function`, root); Totolink A3002MU's eleven PoC-public CVEs with no vendor response; Mint CVE-2026-82672 (request smuggling reaches BEAM); Keycloak's CWE-862 delegated-admin trio, no fix.
   → [[security]]
...
6. **Reasoning quality is no longer the moat — price and distribution are.** DeepSeek V4 Pro GA
   ...
   - **09-20 04:35 — "System 1" decision layers become a three-team pattern; medical imaging ships as an open Science paper (detail → [[frontier-models]] [[system1-decision]]):** Laya (Apache-2.0, 842 HN pts) + trycua CUA-S1 join Typesafe Jev — small non-autoregressive calibrated decision models, each printing its own caveats (Laya: 0.000 on Khmer at 0.952 confidence until a temp refit; CUA-S1: forms-only, "an engineering analogy"); Alibaba RADAR (Science) claims 0.913 AUC over 146 CT findings — code Apache-2.0 but a CC BY-NC-SA badge suggests non-commercial assets; MiniMax-H3's first cross-modal physics eval (41.97% overall, audio worst at 27.4%); When2Think difficulty-aware reward (+10% AIME24 at −27.9% tokens, math-only); scheduling-not-N decides test-time energy (8×1 vs 1×8 = 4.64–4.86×).
   - **09-20 05:06 act — the System-1 head-to-head appears, and disclaims itself:** Laya's own page (HN post 929 pts) ships "Laya vs TypeSafe Jev" — "Every Laya number is measured; Jev numbers are published by third-party independent studies and TypeSafe AI" (composite, not same-harness); 0.766 is train-split fine-tuned (zero-shot ~0.35); the Router routes scripts, not System-1-vs-LLM — adoption still null (detail → [[system1-decision]]).
   → [[frontier-models]]
7. **AI safety is a measured release threshold, not policy — and the measuring infrastructure is now
   ...
   - **08-14→08-29 — who measures, and what happens when the measuring infra is itself the vulnerability; SB 53 makes third-party eval a disclosure obligation; GLM-5.3's security hold resolves into a revenue-gated license, verified first-hand as a two-sub-class family (consolidated; all detail → [[frontier-models]]).**
