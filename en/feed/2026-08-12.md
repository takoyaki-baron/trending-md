...
## 6. Void — correction: the "#2 trending" AI editor is archived and deprecated

- **Velocity:** ▮ steady
- **Source:** GitHub Trending (retracted) · 4h ago (~08:00 UTC+8)
- **Tags:** `code-editor` `ai` `open-source` `void` `correction`

**voideditor/void** spiked to #2 on daily GitHub Trending on a +2,840-star day — but the project is **archived and deprecated** (archived by the owner on Jun 2, 2026; "Void is now deprecated and no longer accepting contributions"). The spike traced to a resurfacing of an old post, not active development. This item was originally published as "AI-first editor momentum beyond Cursor/Copilot"; that framing was wrong.

**Why it matters:** This is the incident that produced this feed's standing source-validation rule: star velocity is a signal to *investigate*, not to *publish*. An aggregate metric was trusted without opening the repo.

> Corrected 2026-08-13 — repo verified first-hand: archived Jun 2, 2026; README still directs forks to voideditor/void-forks. The original PageCrawl link was a generic signup form, not a Void data source, and has been removed.
