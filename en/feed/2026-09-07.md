...
[`🔗 Sansec: StyleSmuggler`](https://sansec.io/research/stylesmuggler) · [`🔗 The Hacker News coverage`](https://thehackernews.com/2026/09/unpatched-magento-and-adobe-commerce.html)
...
- **Velocity:** ▮▮ rising
- **Source:** GitHub Trending · 3,215 stars · +436 stars today · TypeScript · GPL-3.0
...
Elastic documented four previously unreported programs tied to REVSTEALER — a commercial Windows infostealer (~4,700 VirusTotal matches) — that persist after the stealer deletes itself: ProManager (wallet theft + overlay phishing + keylogging), WinUpdate (clipboard crypto-address swapping + recovery-phrase capture), SoftManager (reverse-proxy turn) and LockAppHost (CMSTP elevation, Defender exclusions, 5 Windows Update services and 11 scheduled tasks disabled, a miner hidden in suspended `nslookup.exe`/`svchost.exe`). Distribution runs through ≥17 hijacked YouTube channels pushing game-cheat lures with AI-generated videos; the "~sold since February 2026" dating (earliest VT sample) and the fake "Claude Opus 5 Free Desktop" lure are Morphisec findings carried in The Hacker News's coverage, not in Elastic's report (no indication Anthropic was compromised).

**Why it matters:** the self-deleting stealer + persistent-module split means an infection looks "clean" post-cleanup while Defender exclusions and a miner survive — responders must re-enable services and rotate sessions, not just reset passwords. Key caveat (per The Hacker News's characterization of the report, not an on-page Elastic statement): Elastic never observed any of the four modules delivered onto a live REVSTEALER host — the linkage rests on shared tradecraft, not an observed hand-off — and the public YARA set has no LockAppHost rule.
...
## 26. The NX bit is not just about security — a bare-metal debugging story where NX is what stops speculative instruction fetches

- **Velocity:** ▮ steady
- **Source:** Hacker News · 35+ pts · 23 comments · ~2d ago (~10:00 UTC+8 Sep 5)
- **Tags:** `nx-bit` `cpus` `arm` `memory-safety` `systems`

A guest post on purplesyringa.moe walks through debugging a bare-metal ARM64 hypervisor for postmarketOS on a MediaTek MT6735 (Cortex-A53): with the `CTR_EL0` intercept enabled, the phone hung at random. The chase ends at the difference between `blr x0` (dynamic, mispredictable) and `bl` (static): the CPU's speculative *instruction* fetches — not data accesses — were reaching address 0x0, the locked-out bootrom, and marking the pages non-executable is what stopped them. The upshot, quoting ARM's own docs: Device memory prevents speculative data accesses only, so on ARM the NX bit is "not about security at all — it's essentially an attribute bit" (the author searched the Cortex-A53 errata and found nothing similar — spec-conformant, not a silicon bug).
