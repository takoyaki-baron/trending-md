...
## 17. DaVinci Resolve 21.1 — Blackmagic ships 100+ new tools, and the real AI headline is assistants driving the editor

- **Velocity:** ▮▮ rising
- **Source:** Blackmagic Design release Sep 8 (read in full) · HN 367+ pts · ~14h ago (~22:30 UTC+8)
- **Tags:** `davinci-resolve` `video` `fusion` `release` `ai-tools`

Blackmagic announced DaVinci Resolve 21.1 on Sep 8: over 100 new tools and controls across the edit, color, and Fusion pages, 25 new Krokodove-based shape and 3D tools in Fusion, expanded multi-cam switching (25 angles), individual Dolby Vision/HDR10+/HDR Vivid MultiMaster trims, and 20 new scripting APIs. **Corrected in place (2026-09-09 20:31 run):** the release text states the actual AI headline is **AI-assistant integration — Resolve now supports Claude, Claude Code, and ChatGPT Codex** driving project analysis, media organization, highlight-edit creation, and batch rendering in conversational language. The earlier framing ("Neural Engine media search, camera-slate reading, de-aging, blemish removal") appears nowhere in the primary release; it traced to secondary coverage and has been retracted. Velocity kept: the HN momentum attached to the real release, not the retracted strand.

**Why it matters:** the first major Resolve update of the agentic-AI era ships as an *agent-integration* release — a mainstream NLE exposing its workflow to third-party coding assistants is a sharper agent-distribution datapoint than a Neural Engine bump. The correction is also the lesson: the vendor's own free-text release is checkable in seconds, and the check flips the story from "paid-tier ML features" to "your coding agent can run your editor."
