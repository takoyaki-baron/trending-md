...
**Why it matters:** less a product than a demonstration of where the eBPF verifier has landed — bounded loops, arena pointers and tail calls are now enough to run a userspace runtime in-kernel — which matters for the legitimate use (in-kernel data processing without writing C against kernel APIs) more than for the DOOM.
