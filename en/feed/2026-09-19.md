...
Per CNN's exclusive report, an AI-assisted intelligence report that
...
session, and an explicit no-facial-recognition line in the README. Pushed Sep 17;
maptheworld.ai is the creator's "Map the World" newsletter covering the project — a
hosted version is planned at Halfpixel.
...
[`🔗 github.com/bilawalsidhu/gods-eye-view`](https://github.com/bilawalsidhu/gods-eye-view) · [`🔗 creator's "Map the World" newsletter`](https://maptheworld.ai/)
...
cipher from scienceblogs.de's list of 50 unsolved ciphers. The message — an ADFGVX
transmission sent November 27, 1918 — was decoded in full by recovering the documented
historical key `TRUPPENVERSCHIEBUNG` from Childs's *German Military Ciphers 1914–1918*,
rebuilding the transposition arithmetic, and then checking the result against ship logs
(HMS Canterbury at Sevastopol, November 24). Astra's own explanation for why the message
went unsolved — a key change dated after it — remains unverified. The author's hedge is
explicit: "I am not aware of this particular message having ever been decoded before" —
no historian or cipher authority has validated the transcript.

**Why it matters:** The second historical-cipher claim for a frontier model this month
(Fable 5.1 and the Cyphral Distich, Sep 14) — and the first to publish its mechanics
end-to-end: key provenance, rebuilt table, archive cross-check. The interesting signal
is the workflow — a model retrieving the key from published cryptohistory and verifying
its own plaintext against ship logs — but it still stands or falls on the author's
say-so. Read it as a promising self-report, not a solved cipher.
