...
- [ ] **エージェントコンテキストの断片化（ブラウザ vs メモリ）** — ego-lite（「同一ログイン状態・
      隔離Space」）とholaOS（「メモリ＝プレーンテキストファイル」）は、ブラウザアイデンティティ vs
      ディスク上のメモリという異なる側面から、エージェントコンテキストの断片化に挑む。クロスベンダー
      の「エージェントコンテキスト/アイデンティティ」標準が現れるか（アクセスにおけるMCP/A2Aのよう
      に）、それともブラウザアイデンティティとファイルベースのメモリは製品固有のロックインに留まる
      か？→ [[agent-stack]]
