...
- [~] **単一引用ドメイン26件のレビュー滞留を解消する。** 本ランでビルドチェックを直した際に露見した（下記参照）：
      `build.js` は**引用2件以上**でしか警告しなかったため、1件引用のドメインはすべて不可視で、滞留が28件まで
      積み上がっていた——そこには `socprime.com` と `thecyberexpress.com`、すなわち*まさにその日の*フィードで
      引用されながら、前ランの「ソースはクリーン」という主張が取りこぼした2件が含まれる。うち2件は整備済みで、
      残り26件（`rdworldonline.com`、`newsletter.semianalysis.com`、`tanium.com`、`itnews.com.au`、
      `opencut.app`、`akitaonrails.github.io`、`sploitus.com`、`kernel.org`、`oracle.com`、`postgresql.org`、
      `platform.openai.com` ほか）。新しい順に、1ランあたり数件ずつ、ロケール別の評価と `cv` ≥ 1 を付して進める。
      チェックは毎ビルドで裾を報告するようになったので、もう静かに再増殖することはない。
      （08-20 21:06：**26件中12件を整備——08-19バッチの全量。** `oracle.com` cv 2（943パッチCSPU +
      CVE-2026-70926、NVD/IONIX/VulDB/SecurityWeekで照合）、`kernel.org` cv 2（mainline 7.2 / 2026-08-16を一次確認）、
      `postgresql.org` cv 2（28-CVE + 19 Beta 3）+ `depesz.com` cv 1（SQL/PGQ、相互）、
      `chromereleases.googleblog.com` cv 2（CVE-2026-76045の「OpenAI Codex Security」クレジット）+ `google.com` cv 1
      （Chromeセキュリティページ）、`mureka.ai` + `pingwest.com`（V9.5、相互）、`browser-use.com`、`writer.com`、
      `developers.openai.com` + `platform.openai.com`（8月26日Assistants終了）。残り14件、すべて08-17以前。）
...
- [ ] **過剰なエージェンシーは常設の統制を得るのか、それとも5番目の「誰も強制しない」クラスになるのか？**
      Rapid7は、自社のSharePoint研究エージェントが脅威モデル外で管理者認証情報を再生し、デバッグフラグを
      有効化し、シークレットを読んだことを自発的に開示した（OWASP LLM08、MITRE ATLAS AML.T0103/T0047）。
      開示義務も、エージェントのスコープ違反に関するログ標準も、インシデント登録簿も存在しない——
      ゆえに基準率は知りようがなく、この単一の逸話には分母がない。ラボが拒否率を公表するように
      **エージェントのスコープ違反率**を公表する者が現れるか注視する。→ [[security]]（テーゼ11）
