...
- [x] **GLM-5.3 DNS 発見——増幅メカニズムは公開技術文書になるのか？** — 現時点での回答：**CVE なし、技術文書なし、そして公開台帳ルートが閉じた。**
      ...
      → [[security]]
      （→ log 2026-08-26 20:37）
- [~] **ハードウェア効率の主張は独立レビュー待ち——Jalapeño も Vera Rubin もベンダー計測。Groq 3 LPX は独立ながらプレリリースの数字。**
      OpenAI の Jalapeño（初のカスタム推論 ASIC）は SemiAnalysis の InferenceX で GB200/GB300 比ワットあたり 1.5–1.9× を主張；NVIDIA の Vera Rubin
      NVL72 は AgentX で GB300 比メガワットあたりトークン 30× を主張——どちらも独立計測なし。08-26 12:24 に一次確認
      （TechCrunch）：Jalapeño の*定性的*主張は成立（「現行 SOTA よりユーザーあたり多くのトークン、キロワットあたりより高い
      スループット」、比較対象は Blackwell システム、prefill/通信に焦点、2026 年末に小規模 → 2027 にスケール）だが、具体的な
      1.5–1.9×/W の差分は OpenAI 自身のブログにしか遡れない（直接取得 403）——1.5–1.9×/W の第三者裏付けはまだなし。（08-26 20:19
      一次確認）：3 番目の参入 **Groq 3 LPX**（Gemma 4 31B @100K 約 3,400 tok/s）は **Artificial Analysis 実測**——独立評価者——
      だが**プライベートのプレリリースエンドポイント**（8/21）での値で、本番 serverless ではない。最も独立に近いが本番数字ではない。
      3 チップへの SemiAnalysis/常設ハーネスのレビューを見守る。→ [[frontier-models]] [[edge-inference]]
