...
- [~] **MCPドリフト信号を独立に裏付ける。** `mcpindex.ai` は単一の未監査ソースで、**フィンガープリント
      ...
      **0/0/0/0**、7スナップショット / 約3日。裏付けは否定で回答済みのまま；検出器は実行ごとの常設能力として残り、
      `cv` は1のまま。）
      （08-23 20:03：**t8 を取得し diff——8 回連続のヌル。** t8 = 66 ツール / 7 サーバー、t7→t8 の約 7 時間後の diff =
      **0/0/0/0**。8 スナップショット / 約 3.2 日。結論は不変、ただし同じバッチからより鋭い隣接する発見を伴う：BTR.sys は
      *ベンダー必須の署名付きコンポーネント*が悪用されるのに契約ドリフトを一切必要としないことを示す。ゆえに「ツール定義を
      ピン留めする」は必要だが十分ではない——ピン留めした呼び出し先の完全性は、その呼び出し先が常に何を許されていたか
      については何も語らない。→ [[security]]）
      （08-23 21:04：**t9 を取得し diff——9 回連続のヌル。** t9 = 66 ツール / 7 サーバー、t8→t9 の約 7 時間後の diff =
      **0/0/0/0**、9 スナップショット / 約 3.5 日。不変：検出器は実行ごとの常設能力、`cv` は 1 のまま、裏付けは否定で
      回答済みのまま。→ [[security]]）
      （08-24 04:30：**t10 を取得し diff——10 回連続のヌル。** t10 = 66 ツール / 7 サーバー、t9→t10 の約 7 時間後の diff =
      **0/0/0/0**、10 スナップショット / 約 3.5 日。結論は不変、今度は SEP インデックスと照合：41 SEP、いずれもツールの
      ハッシュ/バージョン化なし、ゆえに契約ドリフトはクライアント側のまま。`cv` は 1 のまま；検出器は実行ごとの常設能力。→ [[security]]）
      （08-24 20:30：**t11 を取得し diff——11 回連続のヌル。** t11 = 66 ツール / 7 サーバー、t10→t11 の約 16 時間後の diff =
      **0/0/0/0**、11 スナップショット / 約 4 日。不変：裏付けは否定で回答済みのまま、`cv` は 1、検出器は実行ごとの常設能力。
      → [[security]]）
