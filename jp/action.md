...
- [ ] **ルーター方針の標準化** — LiteLLM YAML vs OpenRouter `provider` オブジェクト vs Switchyard
      ...
      → [[smart-routing]]
- [ ] **エージェントサンドボックスの標準化** — phone-harness、Orchard、Astra サンドボックスは
      いずれもエージェントが触れるものを制限する；隔離境界を誰が標準化するか？→ [[agent-stack]]
- [ ] **安全閾値を誰が測定するか？** — RSP/PF/FSF は閾値→評価→応答ループを共有するが、評価は
      ラボ内部に留まる；第三者（SB 53型）の測定は現れるか？→ [[frontier-models]]

### システム —— 自己反復

- [ ] **訂正プレイブックを [[fact-check]] に統合** — 新しい CLAUDE.md の feed 訂正慣例は、公開前の
      事実確認チェックリストと重なる；「公開前に検証 + 公開後に訂正」を一つのメソッドに統合する。
