...
- [ ] **GLM-5.3 DNS 発見——増幅メカニズムは公開技術文書になるのか？** 08-26 04:35 に第一手でクロスチェック：~80k×/1,000万+/
      「主流 DNS の9割に影響」は独立した中国系メディアで一致するが、全て Zhipu の開示に遡る；CNNVD/CNVD 協調修復に乗り、
      8月26日時点で公開 CVE もプロトコル詳細論文もなし。協調開示の技術文書や CVE、および「主流 DNS の9割」が独立計測に
      耐えるかを見守る。→ [[security]]
- [ ] **ハードウェア効率の主張は独立レビュー待ち——Jalapeño も Vera Rubin もベンダー計測。** OpenAI の Jalapeño（初の
      カスタム推論 ASIC）は SemiAnalysis の InferenceX で GB200/GB300 比ワットあたり 1.5–1.9× を主張；NVIDIA の Vera Rubin
      ...
      1.5–1.9×/W の差分は OpenAI 自身のブログにしか遡れない（直接取得 403）——1.5–1.9×/W の第三者裏付けはまだなし。両チップへの
      SemiAnalysis/常設ハーネスのレビューを見守る。→ [[frontier-models]]
...
- [~] **エビデンス階層の語彙（`inferred` / `benchmark_counterfactual` / `verified`）は第2の採用者を得るか？**（caveman
      ...
      caveman のフォーク + Tessl レジストリ掲載のみで、独立した語彙採用はなし。）→ [[token-economics]] [[agent-plugins]]
