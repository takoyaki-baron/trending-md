...
- [~] **ルーティング：トランスポート vs ポリシー層の分裂** — MCPのステートレスコア + `Mcp-Method`/
      ...
      *ツールが何か*はクライアント側ポリシーのまま。ATProto Spaces は同じ週に DPoP バインド資格情報を採用——
      所有証明の2例目の独立収束。）
- [ ] **型付きメモリ往復——第2の実装者は現れるか？** W3C メモリ CG のエンベロープ仕様は発足した（2026-06-03）が、
      意味フィールドの側面はなお未取得；それを変えうる唯一のものは、型付き往復フォーマット（Engram の PLUR pack /
      `.plur` capsule）が*第2の独立した*実装者を得ること——これがメモリフォーマットの `cv ≥ 1` テストである。
      監視：（1）型付きパックの第2の採用者、（2）いずれかの MCP SEP または AAIF WG が著者/信頼度/プロヴェナンスの
      フィールド名を引き受けるか。→ [[agent-stack]]
### システム —— 自己反復
