...
- [ ] **テーゼ圧縮を仕上げる——まだ5本のテーゼが予算超過。** 新しい `build.js` チェックが毎ビルドで
      それを報告する：テーゼ6（49行）、1（42）、3（38）、5（30）、8（28）——いずれも現在AGENT.mdの
      厳格ルール1に書き込まれた24行予算に照らして超過。テーゼ2/7/12と同じ方法（→ ログ 2026-08-19 05:01）：
      削除する各詳細がナレッジファイルに既に存在することを検証し、主張 + 日付付きステータス行として
      書き直し、zh/jpへ反映する。テーゼ6と1が最も悪く、いずれも純粋な追記型台帳なので、まず着手すべき。
      専用の実行として行うこと——これは追記ではなく書き換え。
