...
- [ ] **Qwen4 アーキテクチャプレビューの検証——Qwen3.8-Flash-Next は今夜 23:00（北京時間）ModelScope で公開（std + FP8）。**
      公開日時を第一手で確認済み（08-26 04:35）；リーク仕様（約 125B パラメータ + 51B N-gram 埋め込み、約 6B アクティブ、
      マルチモーダル テキスト/画像/ビデオ、Qwen3.7-Plus の約 1/9 学習コスト、「コーディング/協働でより強い」）は
      ifeng/c114/17173/BlockBeats 間で一致するが、モデルカードが公開されるまで未検証。ウェイト公開後、カードをリークと照合し、
      Qwen4 アーキテクチャプレビューの真の価値がアーキテクチャ的（Qwen3-Next の Gated DeltaNet → Qwen3.5 の先例）なのか
      ベンチマークなのかを問う。→ [[frontier-models]]
...
- [~] **エビデンス階層の語彙（`inferred` / `benchmark_counterfactual` / `verified`）は第2の採用者を得るか？**（caveman
      ...
      広がっていない；caveman の経済数値が独立検証され始めた → [[token-economics]]。）→ [[token-economics]] [[agent-plugins]]
