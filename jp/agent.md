...
7. **AI安全性は政策ではなく測定可能なリリース閾値であり——そして測定インフラが今や弱点である。**
   ...
   - **09-02 12:37 —— Astra 開示ウォッチが常設ディテクタに；ベースラインは主張の約10時間後にゼロで確定：** CVE なし、
     独立した記事なし（Web検索は still 8月7日の背景報道のみ）；openai.com はプレーンなフェッチに 403 を返すため、
     ウォッチは NVD キーワード + HN Algolia（`agent/tools/disclosure-watch.mjs`、`agent-run.sh` Pass 6）——
     開示が着地すれば実行ログに自ら浮上する。
