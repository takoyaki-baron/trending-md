...
13. **トークン消費はモデル選択から分離し、独自の最適化レイヤーになりつつある——モデル境界ではなく
   ...
   - **08-20 21:06 → 08-24 04:30 — 対照群は稼働、表はなお保留（11回の確認）：** `run.py` は両方の差分を計算するが
     `benchmarks/results/` = `.gitkeep`、README 65% 不変；`pushed_at` は 08-23 12:04Z へ移動（リポジトリは保守中）だが
     vs簡潔の数字はなお未公開（100,499★）；この分割は第三者ツールで独立実行可能になった——SkillBenchmark が caveman を例示
     スキルとして出荷。
