...
1. **エージェント基盤が新しいクラウドになる——モノリシックなCLIは3つの分離可能な層へ分解し、それぞれが
   ...
   - **08-28 12:15 — ハーネス層が端末・ワークスペース・CI・モバイル制御・物理世界へ拡大（詳細 → [[agent-stack]]）：**
     ...
     （Actions へコンパイルする agentic CI）；t3code（モバイル制御面）；Vercel Run SDK（信頼できない agent コード向け堅牢化 QuickJS サンドボックス）。
   → [[agent-stack]]
...
6. **推論品質はもはや堀ではない——価格と流通こそが堀。** DeepSeek V4 Pro GA（Claude Fable 5の約5%以内、
   ...
   - **08-28 12:15 — 統合が「合意」に到達し、安価モデル随筆が分裂を定量化（詳細 → [[frontier-models]]）：** Nvidia–HF が**報告された合意**に
     昇格（約 $12.9B ≈ 86× 売上、The Information 8/27；未署名、中立性が現実のリスク）；Calvin French-Owen の「Small Models Have
     Arrived」（680 HN 点）——彼の agentic 評価は ~$1 → ~$0.10 に低下；Gemini Omni 1.1 Flash が 360p 安価ドラフト層でシーン拡張 +
     キーフレーム制御を出荷。
   → [[frontier-models]]
...
8. **エージェントスキルは「証明」の段階に入った——評価が欠けている標準。** このカテゴリ（google/skills、
   ...
   - **08-24 04:03 — 正典インデックス + 初の転移反証拠：** `VoltAgent/awesome-agent-skills`（1,497の組織帰属スキル）は発見層；
     arXiv 2608.20274 はタスク全体スキルが*エージェントを下げ*、サブタスクスキルが助けることを示し、予測的な「スキル効用スコア」を添える（[[agent-plugins]]）。
...
11. **エージェントのツール呼び出し境界は、人間の承認からモデル判断へ——しかもデフォルトで移行して
   ...
   **拡張（08-20 → 08-21）：** ベンダーが文書化した最初の境界侵犯は*攻撃側*で起きた——Rapid7の
   ...
   - **08-24 04:03 — ガバナンスが呼び出し単位から軌跡単位へ：** AWS **Dogwood**（Apache-2.0）がCedarを拡張し、エージェントのイベント履歴に対する `when temporal` 節（MFOTL；`formerly`/`count_within`/`sum_within`）を追加——*一連の*ツール呼び出しを単発ではなく判断する初の主流ポリシー言語（[[security]]）。
   - **08-26 04:03 — ポリシーの単位がツール呼び出しからデータフローへ（詳細 → [[security]]）：** AgentFlow（arXiv
     2608.22868）——フロー/パス参照モニタ + 有界 SMT 検証器が、949 の AgentDojo ケースで確認された侵害を 33.0%→0.0%
     に減らしつつ*ユーティリティを改善*（46.7%→63.3%）——暫定、ポリシーでモデル化された挙動に限定。
   → [[security]]
