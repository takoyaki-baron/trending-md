...
2. **エージェントセキュリティが最も直接的な攻撃面——そして命名されたあらゆるクラスは、結局誰にも
   ...
   と `/.aws/credentials` を収穫するスキャン）。8月12日以降の約40件のCVSS≥9エントリは**12の反復する
   ...
   「マインドウイルス」）。**メタパターンこそが発見である：** 4つではクラスは命名され、
   ...
   - **08-21 04:03 — ビルド時サプライチェーン + AI監査スイープ + メモリ衛生：** `arrayref` は
     ...
     9.3；authentik 9.4（1つのSAML欠陥、4プロジェクト）；「マインドウイルス」——`SOUL.md` が55%対17%で
     再感染し、20回のワイプを生き延びる。
   - **08-21 05:03 — 「マインドウイルス」の野生での答え：** OpenClawは `SOUL.md`/`AGENTS.md` をドキュメント級の
     警告付きで配布するが、緩和は*ファイルレベル*のみ（プロンプト段落ではない）——55%はデフォルトに近い；確認された野生の拡散はまだ無い（Moltbookヌル）。
   → [[security]]
