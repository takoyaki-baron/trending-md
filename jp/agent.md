...
2. **エージェントセキュリティが最も直接的な攻撃面——そして命名されたあらゆるクラスは、結局誰にも
   ...
   - **09-04 04:03 — 生成コードが攻撃面に、RAG 取り込み層が読み取りプリミティブに（詳細 → [[security]]）：** Orval に
     1 日で 9 件の重大アドバイザリ、根本原因は同一——spec 制御文字列がエスケープなしでテンプレートリテラルへ補間
     （バッククォートで生成されたリクエスト URL リテラルを突破；モジュールレベルのテンプレートリテラルとして出力される
     schema `default` は **import 時 RCE**；開示時点で修正版なし）——OpenAPI ドキュメントは生成クライアントを
...
- **バッチ末尾（09-04 04:03、詳細 → [[security]] [[frontier-models]] [[platform-gatekeeping]] [[edge-inference]]）：**
  ...
  依存が一つのシステムとして全体故障、Ask HN スレッドは 485 コメント）；Cerebras が Qwen3.8-27B を ~1,500 tok/s でサーブ（ウェファー
