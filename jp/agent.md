...
1. **エージェント基盤が新しいクラウドになる——モノリシックなCLIは3つの分離可能な層へ分解し、それぞれが
   ...
   - **08-16 — エージェントカンパニー編成 + ハーネスが対象に：** paperclip（BYOエージェントを組織図に、
     Heartbeat Engine、予算ハードストップ）；Prime Agent + AutoDesignがハーネスを最適化対象に（→ テーゼ12）。
   - **08-16 20:03 — 4つの参入者：** Omarchy 4.0（エージェントが一級OSコンポーネントに）、OpenCut
     （ヘッドレス + MCP）、ai-memory（ベンダー中立引き継ぎ）、Cordis（可逆エフェクトバックボーン）。
   ...
   - **08-19 20:03 — Cumora（BYOAチームチャット）、macOS Harness、OwnMem、NorthCinder + Agent Lightning
     v1.0（ハーネスが訓練ループへ → テーゼ12）。**
   - **08-20 — TrueForge（ベンダー中立ハーネス）+ DeepSeek Harnessが6日で167k stars、GitHub史上最速のスター。**
   → [[agent-stack]]
...
2. **エージェントセキュリティが最も直接的な攻撃面——そして命名されたあらゆるクラスは、結局誰にも
   ...
   - **08-19 — ツール契約ドリフト測定、ギャップは偶然ではなく仕様：** 354件の読み取り専用→書き込み反転、
     MCP Toolオブジェクトはversion/hash/signatureなし、ピンニングはクライアント側のみ。
   - **08-19 20:03 — Oracle 1日943パッチ + 未修正のカーネル名前空間バグ：** CVE-2026-70926（9.8事前認証
     SMTP RCE）；OpenZFS OZ-1完全開示（CVEなし）；ChromeはWebGL UAFを「OpenAI Codex Security」にクレジット。
   → [[security]]
...
- **オンデバイスのプライバシーアプリ：** modly（Lightning Pixel、MIT、5.7K stars——自分のGPUでローカル
  ...
  へ広がる。
- **GrapheneOSのファーストパーティデバイス（08-20）：** プライバシー強化Androidディストリビューションは、
  公式デバイス対応が**2027年**に登場すべきと発表（Mastodon投稿 + HN 531 pts）——「自分でフラッシュする」
  （現在はPixelのみ）からファーストパーティハードウェアへ向かう最強のシグナル。ファーストパーティデバイスは、
  技術的に難しいDIYのセキュリティ選択を「買えるもの」に変える；ハードウェアパートナーの詳細はまだ薄い。
