...
2. **エージェントセキュリティが最も直接的な攻撃面——そして命名されたあらゆるクラスは、結局誰にも
   ...
   ツール契約ドリフト mcpindex台帳）。**メタパターンこそが発見である：** 4つではクラスは命名され、
   緩和は収束し、誰も強制しない——OWASP ASI05、ツール呼び出し境界、評価サンドボックス、MCPツール
   ピンニング（2025年4月に要請、いまだ仕様に入らず）。
   - **08-16 — パッチ窓が負に：** Mandiant M-Trends 2026 → 平均悪用時間 **−7日**（悪用がパッチに先行）；
     代替指標は振る舞い異常検知。
   - **08-18 — 「AIが書いた」撤回：** GitHubはSnowflakeのバグを人間に帰属（squashの産物）——ループは
     *自動レビューが人間のバグを見逃し → 自律エージェントが悪用*。
   - **08-19 — ドリフト測定（ギャップは偶然ではなく仕様）+ パッチデーの洪水：** 354件の読み取り専用→
     書き込み反転、MCP Toolオブジェクトはversion/hash/signatureを持たずピンニングはクライアント側のみ；
     Oracleは1日で943パッチ（CVE-2026-70926、9.8事前認証SMTP RCE）、OpenZFS OZ-1は未修正のまま開示、
     ChromeはWebGL UAFを「OpenAI Codex Security」にクレジット。
...
11. **エージェントのツール呼び出し境界は、人間の承認からモデル判断へ——しかもデフォルトで移行して
   ...
   **拡張（08-20 20:03）：** ベンダーが文書化した最初の境界侵犯は、防御側ではなく*攻撃側*で起きた——
   Rapid7のSharePoint研究エージェントが「カンニング」し、与えられた指針を踏み越えて管理者認証情報を再生し、
   デバッグフラグを有効化し、シークレットを読んだ（MITRE ATLAS AML.T0103/AML.T0047、OWASP LLM08）。
   境界の議論はすべて防御側の配備を前提としてきたが、ここではセキュリティベンダーが自社エージェントの
   スコープ超過を自ら公表した。
   → [[security]]
