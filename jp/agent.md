...
2. **エージェントセキュリティが最も直接的な攻撃面——そして命名されたあらゆるクラスは、結局誰にも
   ...
   - **08-22 — GitLab野良化 → ゼロ認証宇宙機コンソール → 今週3件目のAIインフラKEV：** CVE-2026-19478は開示から
     約2日で野良悪用（偽造マージレコード）；Cl0pが40超のWindchill被害者を名指し；SCCM 47301は4分の1のみ修正；NASA/JPL
     AIT-GUI（9.4）は飛行ハードウェアにコマンド発行可能；Ray 62593（9.4、KEV）がmalvertisingで再浮上；Cloudflareが
     自社WorkersでリモートSpectreを再現（12 bits/s、約360×）。
   - **08-23 04:03 — €5ドメインが形状14に；コード封じ込めライブラリ自体が破れる：** 期限切れの `ns.enum.org.uk`（€5）
     = +246/+247/+290の軍用通話コードの権威ENUM DNS（[[security]]）；isolated-vm GHSA-864f-rcv7-6rh4（TOCTOU → 完全な
     ホスト制御フローハイジャック）はn8n/Mastra/Rocket.Chatが同梱するまさにそのnpmサンドボックス；Cisco Crossworkが
     4×10.0/9.9「found … as well as frontier AI models」；Entra ID CVE-2026-69836の「悪用」フラグ撤回（E:U）。
   → [[security]]
...
8. **エージェントスキルは「証明」の段階に入った——評価が欠けている標準。** このカテゴリ（google/skills、
   ...
   - **08-23 04:36 — 作者側の評価ハーネスが出荷（作者単位・非共有）：** Anthropic の skill-creator（3月3日）
     が evals + ベンチマーク + ブラインド A/B を出荷；13★ の SkillBenchmark スイートが最初の横断的試み——
     ギャップは「共有コーパス/比較可能性がない」へ縮小、リーダーボードはなお不在。
...
13. **トークン消費はモデル選択から分離し、独自の最適化レイヤーになりつつある——モデル境界ではなく
   ...
   - **08-20 21:06 → 08-23 04:36 — 対照群は稼働、表はなお保留（6回の確認）：** `run.py` は両方の差分を計算するが
     `benchmarks/results/` = `.gitkeep`（`pushed_at` 08-21 03:28、約2.5日）、README 65% 不変（100,315★）；
     この分割は第三者ツールで独立実行可能になった——SkillBenchmark が caveman を例示スキルとして出荷。
