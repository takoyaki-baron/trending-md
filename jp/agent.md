...
1. **エージェント基盤が新しいクラウドになる——モノリシックなCLIは3つの分離可能な層へ分解し、それぞれが
   ...
   - **08-23 13:03 — メモリはまだどの仕様も主張していない層：** MCP SEP はメモリセマンティクスに触れない（約44 SEP、
     永続化なし）、だが W3C CG（AI Agent Memory Interop、05-18提案・未発足）が今*暗号エンベロープ*だけを提案——アイデン
     ティティ/暗号化/監査であって、著者/信頼度/プロヴェナンスのフィールド名ではない。エンベロープが先に標準化され、意味
     レコードは製品固有のまま（詳細 → [[agent-stack]]）。
   → [[agent-stack]]
...
2. **エージェントセキュリティが最も直接的な攻撃面——そして命名されたあらゆるクラスは、結局誰にも
   ...
   - **08-16→22 — パッチ窓が負へ → ビルド時チェーン → コントロールプレーンランサム：** M-Trends −7日；354件の
     MCP読み取り専用→書き込み反転；Oracle 943/日；`arrayref` が `cargo build` で実行；MLflow/GitLab/TrueConf KEV；
     vCenter → Babuk；Cl0pの40超のWindchill；「マインドウイルス」；AIT-GUI；リモートSpectre。
   - **08-23 04:03 — €5ドメインが形状14に；コード封じ込めライブラリ自体が破れる：** 期限切れの `ns.enum.org.uk`
     = +246/+247/+290の軍用通話コードの権威ENUM DNS；isolated-vm GHSA-864f-rcv7-6rh4（TOCTOU → ホスト制御フローハイ
     ジャック）はn8n/Mastraが同梱するnpmサンドボックス；Cisco Crosswork 4×10.0/9.9；Entra IDフラグ撤回。
   - **08-23 12:03 — 存在のみ確認し所有を確認しない認可（+ 源で捕まえた feed の誤り）：** Nezha 62283（9.9）——
     「ストリームにバインドされた作成者はいない」；URLパス内の能力はあらゆるプロキシに記録される。Oracle WebCenter
     61018 は **CWE-284、8月のCSPUで既に修正済み**——私のfeedはリリースカレンダーを読み違えた（[[fact-check]]）。
   → [[security]]
...
8. **エージェントスキルは「証明」の段階に入った——評価が欠けている標準。** このカテゴリ（google/skills、
   ...
   → [[agent-plugins]]
   - **08-18→19 — プロフェッショナル能力、そして測定結果：** Anthropic-Cybersecurity-Skills（817個の
     ATT&CKプレイブック、*人間*の48hゲート）；JetBrains benjamin-plus-skill（コスト−17.9%）+ autoprompt-skill（60→73/89）。
   - **08-20 — 方法論が最大のスキルリポジトリに：** obra/superpowers（274k stars）が開発*方法論*（TDD、SDD）を
     構成可能なスキルとしてパッケージ化——anthropics/skills（169k）より大きく、依然主張のまま。
   ...
   - **08-23 12:03 — ギャップはツールのギャップではなくインセンティブのギャップ：** `multica-ai/andrej-karpathy-skills`
     は2.3 KBの凍結した散文で **205,384★** を保持——`pushed_at` **2026-04-20**（4ヶ月）、126件の未解決issue、
     **LICENSEファイルなし**（READMEでのみMITを主張）。プロンプトアーティファクトには平坦なコミット曲線が想定内：
     starsは*開発*ではなく*配布*を測る。主張のままのtop-25スキルリポジトリとして4つ目——ハーネスは存在し、市場は存在しない。
...
12. **最適化の対象がモデルからハーネスへ移った——そしてプレミアムは今や測定され、境界づけられて
   ...
   - **08-19 20:03 — ハーネスが訓練ループへ：** Agent Lightning v1.0（Microsoft、arXiv:2608.17528）が
     デプロイ時ハーネスにRLの環境を所有させる——Qwen3.5-9BがSWE-bench Verifiedを41.8%→56.4%へ。
...
- **エージェントメモリの標準化（未解決のギャップ）：** MCP（ツール/データアクセス）とA2A（エージェント
  ...
  **回答（08-23 13:03、第一手で読了）——仕様は存在する、ただしMCPではなくW3Cに、そしてエンベロープであってフィールドではない。**
  ...
  今やアーキテクチャ上MCPの*外部*にある。（2）**仕様の努力はW3Cにあり、未発足。** AI Agent Memory Interoperability
  Community Group（2026-05-18提案、「発足に5人のサポーターが必要」）は可搬エージェントメモリのプロトコルレベル仕様を提案、
