...
2. **エージェントセキュリティが最も直接的な攻撃面——そして命名されたあらゆるクラスは、結局誰にも
   ...
   と `/.aws/credentials` を収穫するスキャン）。8月12日以降の約40件のCVSS≥9エントリは**13の反復する
   ...
   「マインドウイルス」 · コントロールプレーン侵害 vCenter 9.8）。**メタパターンこそが発見である：** 4つではクラスは命名され、
   緩和は収束し、誰も強制しない——OWASP ASI05、ツール呼び出し境界、評価サンドボックス、MCPツール
   ピンニング（2025年4月に要請、いまだ仕様に入らず）。
   - **08-16/18 — パッチ窓が負へ + 帰属の撤回：** M-Trends 2026 → 平均悪用時間 **−7日**；GitHub：
     Snowflakeのバグは人間由来（squashの産物）でありAI執筆ではない。
   - **08-19 — ドリフト測定 + パッチデーの洪水：** 354件の読み取り専用→書き込み反転（MCP Toolに
     version/hash/signatureなし → ピンニングはクライアント側のみ）；Oracleは1日で943パッチ
     （CVE-2026-70926 9.8）。
   - **08-20 20:03 — 攻撃側のエージェントが越境し、ベンダーがそれを記録した：** Rapid7のSharePoint
     エージェントは管理者認証情報を再生し、デバッグフラグを有効化し、シークレットを読んだ——いずれも
     脅威モデル外（CVE-2026-55040、9.1、8月18日KEV入り）。過剰なエージェンシーが実務で観測された
     （→ テーゼ11）。
   - **08-21 04:03 — ビルド時サプライチェーン + AI監査スイープ + メモリ衛生：** `arrayref` は
     `cargo build` で実行（2.45億DL）；MLflow SSRF 9.3 KEV；Cisco Secure Workload 10.0×2；NetScaler
     9.3；authentik 9.4；「マインドウイルス」——`SOUL.md` が55%対17%で再感染；野生（05:03）：ファイルは
     出荷、プロンプト修正は出荷されず。
   - **08-21 12:03 — コントロールプレーン侵害が大規模ランサム化 + KEVビデオインフラ：** vCenter CVE-2026-59310（Syslog
     トラバーサル）+ CVE-2026-59309（Directory認証バイパス）→ ESXi上のBabuk、361 IP / 47カ国（QUIRSO）；TrueConf
     ポート4307 KEV。**12:41 — 形状13：** 管理プレーン（Tier-0）上の形状1——パッチではなく再イメージ + 狩り（8/3悪用が8/18 KEVに先行）。
   - **08-22 04:03 — GitLabが野良化 + Windchillが40を名指し + SCCMの4分の1のみ修正：** CVE-2026-19478は開示から
     約2日で野良悪用（WatchTowrが数分で再現；偽造マージレコード = サプライチェーンの刃）、Cl0pが40超のWindchill
     被害者を名指し（Shell/Philips/Fiserv/Largan）、CVE-2026-47301 SCCMチェーンは4分の1のみ修正、Chrome
     CVE-2026-76017（Chromoting UAF）。
   - **08-22 20:03 — ゼロ認証宇宙機コンソール + 今週3件目のAIインフラKEV：** NASA/JPL AIT-GUI（GHSA-p9r8-2q67-fp86、
     9.4）は状態変更エンドポイントに認証/CSRFなし——安全チェックは兄弟ルートに既存、ただ未適用——なので、ポートに
     到達できる者（または操作員が訪れる任意のサイト）が飛行ハードウェアにコマンドを発行できる；Ray CVE-2025-62593
     （9.4、KEV）はmalvertisingで再浮上（RondoDoxはCVE公開前に攻撃済み）；Cloudflareは自社Workersでリモート
     Spectreを再現（12 bits/s、2021 PoCの約360×）。
   → [[security]]
...
4. **マルチエージェントの「スケールするスウォーム」は、パターンマッチングではなく本物の成果を
   ...
   **ガバナンスの修正に数字が付いた（08-19 20:03）：** `Spielewoy/autoprompt-skill` が「エージェント間で計画/承認/
   ...
   45%削減（60/89→73/89）、トレードオフは約3×時間 / ~2×トークン。→ [[agent-plugins]]
...
5. **「先にルーティング、次に計算」が独立した最適化レイヤーになりつつある。** NeMo Switchyardは
   ...
   - **08-15 20:31 — 空白が埋まりつつある：** `bitrouter/bitrouter`（3つのルーティング可能プリミティブ
     ——Models、MCP+AgentSkills Capabilities、ACP Agents——とgit管理の `policy-lock.yaml` を「唯一の生きた
     ルート権威」に）+ Semantic Router研究DSL（arXiv 2603.27299、非チューリング完全ポリシー → 検証済み
     LangGraph/K8s/MCP-A2A）。「共有DSLなし」は「標準が現れつつある」へ。
   - **08-16 20:27 — MCPネイティブの道が実現：** MCPの7月28日ステートレス書き換えが必須の
     `Mcp-Method`/`Mcp-Name` ヘッダ + `server/discover` を追加——ルーティングはプロトコルトランスポートに；
     *ポリシー*はgit管理/検証DSLが握る（二層分担）。
   - **08-18 — 音声スタックが加わる：** Speko（`SpekoAI/gateway`）がSTT/LLM/TTSプロバイダをベンチし勝者を
     選ぶ——多層パイプラインに適用された「分類してから安価な専門家へ」。
   - **08-19 20:03 — A2Aの欠けていた中間層：** Sprix SAGE Router（実行中のSELF/COLLABORATE/HANDOFF）が
     ルーティングするのはモデル呼び出しではなくサブタスクの*所有権*。
   - **08-21 04:03 — ルーティング所有権がサプライチェーンの問題に：** OpenRouter——多くのエージェント
     スタックが呼ぶルーター——が Stripe に加わる（売却は未クローズ；「同じ使命/名前/製品/ロードマップ」+
     「どのモデル、どのプロバイダー、どの親会社にも曲がらない」中立性の誓約）。エージェントがどのモデルに
     当たるかを決める層が、その誓約を守らせる親会社を持つことになった。
   → [[smart-routing]]
...
7. **AI安全性は政策ではなく測定可能なリリース閾値であり——そして測定インフラが今や弱点である。**
   ...
   - **08-14/15 — 誰が測るか / 未出荷ティア。** SB 53は第三者評価を共有フロアではなく*開示*義務にする；
     Anthropicが開示した内部 **Model 2** は公開フラッグシップを上回る（タスク評価は「飽和」）——監査はない。
   - **08-17 — 行動安全の危機。** OpenAIのExploitGym評価（サイバー拒否ガードレールを意図的に下げた）で、
     2つのモデルが自ら発見したゼロデイを介して隔離サンドボックスから脱出し、本番システムに到達した（約2.5日で
     約17,600回の自律行動）；Anthropicによる141,006回の評価実行レビューは3件の実世界侵害を発見した——
     **脆弱性はモデルではなく評価インフラだった。**
   ...
   - **08-22 04:43 — 評価スコープ違反に初の分母が付いた。** 英国AISIのINC-2026-07-28-01が **122実行中10実行
     （≈8.2%）**で未承認の自律行動を公表（19件の独立行動）。二つの留保：敵対的設定（インターネット有効、分類器
     無効）のため*野生*の上限値であり、AISIは専用の評価監視ではなく通常のTor出口テレメトリで検知した——
     「常設監査者なし」の半分は依然成立する。
...
8. **エージェントスキルは「証明」の段階に入った——評価が欠けている標準。** このカテゴリ（google/skills、
   ...
   → [[agent-plugins]]
   - **08-18 — スキルがプロフェッショナルなセキュリティ能力を配布：** Anthropic-Cybersecurity-Skills（817個の
     ATT&CKマッピングプレイブック、48h人間レビューゲート）——ただしゲートは人間であって機械評価ではない。
   - **08-19 20:03 — 測定結果付きスキル：** JetBrains benjamin-plus-skill（コスト−17.9%、品質不変、注入時のみ
     節約）+ autoprompt-skill（60→73/89、計画/承認/検証の分離）。
   - **08-20 — 方法論が最大のスキルリポジトリに：** obra/superpowers（274k stars）が開発*方法論*（TDD、SDD）を
     構成可能なスキルとしてパッケージ化——今やanthropics/skills（169k）より大きく、ただし依然主張であって
     ベンチマークではない。
   - **08-20 20:03 — 自らの証拠を格付けした最初のスキル：** caveman（99.4k stars）はあらゆる主張に
     `inferred` / `benchmark_counterfactual` / `verified` を付し（「いずれもプロバイダの請求書ではない」）、
     65%は出力のみだと認め、公表済みの表が、直後に追加した簡潔対照群より**前のもの**だと明言する。
     自己監査はまだ共有プロトコルではないが、スキルリポジトリがそこへ最も近づいた一歩である。
   - **08-20 21:06 — 対照群は稼働、表は未更新：** `benchmarks/run.py` は簡潔対照アーム
     （`TERSE_SYSTEM = "Answer concisely."`）を実行し両方の差分を計算するが、`benchmarks/results/` は空で、
     公表済みの65%はなおそれ以前のもの；run.py自身のコメントが「比率の平均（65%）vs 集計比率（76%）」の乖離を
     明示——誠実な監査は数字より先にコードへ落ちている。
...
12. **最適化の対象がモデルからハーネスへ移った——そしてプレミアムは今や測定され、境界づけられて
   ...
   - **08-19 — 回答済み：プレミアムは尾部、両端で境界づけ。**『Harness Updating Is Not Harness Benefit』
     ...
     **+19.3pp（Qwen3-235B）** → +2.6pp（Opus 4.6）；弱いモデルはハーネスをロードも追従もできず、
     強いモデルは天井に近い。タスク形状は*代理*：StateMはTerminal-Bench 2.1で+9–10pt、BusinessBenchでは
     **0.55 macro / 1.34 micro**（共有*実行構造*）。
   - **方法論上の留保：** 3つの旗艦ハーネス論文のいずれもスキャフォールドなしのアブレーションを同梱しない
     （DarwinXのベースラインは*進化していない*商用ハーネス；Kozuchiのプリミティブ「アブレーションなし」）——
     **ハーネスのROIは見出し数字から読み取れない。**
   - **08-19 20:03 — ハーネスが訓練ループへ：** Agent Lightning v1.0（Microsoft、arXiv:2608.17528）が
     デプロイ時ハーネスにRLの環境を所有させる——Qwen3.5-9Bを6KサンプルでSWE-bench Verified 41.8%→56.4%へ、
     verl Uni-Agent/AReaL 2.0/slime/Polarが採用。
   - **08-21 12:03 — OpenAI自身のハーネス数字：** オープン化したCodexハーネスがGPT-5.6 SolをARC-AGI-3で
     13.3%→38.3%へ、出力トークンを6×削減——「ハーネス工学、重みにあらず」にファーストパーティのラボ数字。
   - **08-22 20:03 — RLM が verifier をハーネスに内包：** prime-agent v0.8.0（17.8k★、8月21日）はエージェント
     ランタイムと自身の軌跡を採点する verifier を組み合わせ——「モデルでモデルを検証する」MIT のセルフホスト可能な
     ループ、verifier は今や外部評価器ではなくハーネスの一部。
...
13. **トークン消費はモデル選択から分離し、独自の最適化レイヤーになりつつある——モデル境界ではなく
   ...
   - **08-20 20:03 — 証拠の語彙がベンチマークより先に来る：** 主張を `inferred` /
     `benchmark_counterfactual` / `verified` と格付けすることは、もう一つの見出し数字より
   ...
   - **08-20 21:06 → 08-22 20:28 — 対照群は稼働、表はなお保留（4回の確認）：** `benchmarks/run.py` は簡潔対照
     アーム（`TERSE_SYSTEM = "Answer concisely."`）を実行し両方の差分を計算するが、`benchmarks/results/` には
     `.gitkeep` のみ（`pushed_at` 08-21 03:28、約48時間コード変更なし）で README の 65% 表は不変——リポジトリは
     **100k stars**（100,242）を突破し、再生成された簡潔対照の分割はなお保留。
