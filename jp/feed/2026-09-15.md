...
両モデルともに見逃した検証済みバグが 26 件あった。（09-16 引用訂正：ベンダー原文の URL は現在
404 へリダイレクトされる——上のアーカイブ copy は本稿が引用する全数値を含むことを検証済み。）

[`🔗 entelligence.ai (archived 09-14)`](https://web.archive.org/web/20260914195804/https://entelligence.ai/blogs/gpt-5.6-luna-vs-gpt-6-astra-is-a-1.20-model-good-enough-for-code-review) · [`🔗 HN 議論`](https://news.ycombinator.com/item?id=49703003)
...
今月後半にベータ、2026 年 10 月に正式リリース。Rust 製 NTP クライアントは 27.10 でデフォルト化の
予定。
...
## 46. Redis City：Redis のすべてを歩き回れる 3D モデル——フロントエンドは LLM 製、作者本人が修正した位置づけ
...
**Why it matters:** 09-16 その場で修正：本項目は当初このプロジェクトを「教えるために手作りされ、
隙間を埋めるために生成されたのではない」と評した——しかし作者は HN スレッドで正反対のことを
述べている。「フロントエンドの大部分は LLM で構築した……ライブラリのコードとアーキテクチャに
ついての記事を読み、GPT とローカルモデルで概念を可視化した」とし、今や「こうした可視化を手で
作る意味はない」とも（記事自体は本人が執筆し、ボットが 3 言語に翻訳）。つまりこれはエージェント
生成作品への対重ではなく、その事例研究だ：アーキテクチャの理解は人に、レンダリングの実行は
LLM にあり、教育価値はメッシュを誰が打ち込んだかではなく、コンポーネントごとのソース引用が
概念を正しく説明しているかどうかにかかっている。
