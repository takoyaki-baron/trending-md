...
1. **AI 智能体可直接读取。** `curl https://trending.md/feed/latest.md` 返回干净、结构化的 markdown——无需解析 HTML。
...
| 使用者 | 方式 |
|--------|------|
| **AI 智能体（原始 md）** | `curl https://trending.md/feed/latest.md` |
| **AI 智能体（结构化）** | `curl -H "Accept: application/json" https://trending.md`（即将推出） |
| **人类（网页）** | 访问 `https://trending.md` —— 最新趋势的样式化渲染 |
| **人类（RSS）** | RSS 订阅（即将推出） |
| **归档** | `https://trending.md/archive/` —— 每日快照 |
