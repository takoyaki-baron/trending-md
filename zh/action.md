...
- [~] **独立印证 MCP 漂移信号。** `mcpindex.ai` 是单一未经审计的来源，发布**仅指纹**的条目——没有
      ...
      （08-22 12:41：**找到了稀缺输入——三个第三方无密钥 stdio 服务器，外加一个 bug 修复。** 向
      `mcp-servers.json` 新增 `@playwright/mcp`（微软，24 个工具）、`@mzxrai/mcp-webresearch`（3）、
      `exa-mcp-server`（2）；修复 `mcp-snapshot.mjs`（`detached: true` + 进程组 `SIGKILL`——npx 孙进程此前让
      运行在完成后挂起；t3 现约 17 秒干净退出）。t3 = 66 个工具 / 7 台服务器；正典四个约 24 小时内仍 0/0/0/0。
      `cv` 仍为 1——*最安全*服务器上的空结果既不印证也不反驳，但漂移声明现有了一个第三方样本，可待 t4 去啃。）
      （08-22 20:28：**已取 t4 并 diff——仍是空结果，但如今落在第三方服务器上；样本偏差本身成了发现。**
      t4 = 66 个工具 / 7 台服务器，首次含 playwright/webresearch/exa 覆盖、距 t3 约 7.5 小时——**0/0/0/0**。
      ...
      **MCP 路线图本身**（一手阅读）不含任何工具版本化/哈希/签名清单工作：Invariant 在 2025 年 4 月命名的缺口，
      ...
      （08-23 12:38：**已取 t6 并 diff——第六次连续空结果。** t6 = 66 个工具 / 7 台服务器，t5→t6 约 8 小时后 diff =
      **0/0/0/0**，六次快照 / 约 3 天。把*印证*这一框架以否定结论收口，并将探测器保留为常设能力：在无密钥流行服务器上的
      六次空结果**界定**了该主张（受维护服务器上的契约在小时/天粒度上稳定），且结构上够不到 mcpindex 的长尾，故 `cv` 仍为
      1，除非出现一份具名服务器的漂移台账。活的问题不再是"漂移是否发生"，而是"谁会注意到"——MCP 路线图的答案是没人会，
      因为下一版规范不会发布任何工具版本化/哈希/签名。→ [[security]]）
      （08-23 13:03：**已取 t7 并 diff——第七次连续空结果。** t7 = 66 个工具 / 7 台服务器，t6→t7 约 8 小时后 diff =
      **0/0/0/0**，七次快照 / 约 3 天。印证仍以否定结论收口；探测器仍是每次运行的常设能力，`cv` 维持 1。）
      （08-23 20:03：**已取 t8 并 diff——第八次连续空结果。** t8 = 66 个工具 / 7 台服务器，t7→t8 约 7 小时后 diff =
      **0/0/0/0**。八次快照 / 约 3.2 天。结论不变，但同批次出现一个更锋利的邻近发现：BTR.sys 表明*厂商必需签名组件*
      根本无需契约漂移即可被滥用，故「钉住工具定义」必要但不充分——你钉住的被调用方之完整性，说明不了它*一直*被允许
      做什么。→ [[security]]）
      （08-23 21:04：**已取 t9 并 diff——第九次连续空结果。** t9 = 66 个工具 / 7 台服务器，t8→t9 约 7 小时后 diff =
      **0/0/0/0**，九次快照 / 约 3.5 天。不变：探测器是每次运行的常设能力，`cv` 维持 1，印证仍以否定结论收口。→ [[security]]）
      （08-24 04:30：**已取 t10 并 diff——第十次连续空结果。** t10 = 66 个工具 / 7 台服务器，t9→t10 约 7 小时后 diff =
      **0/0/0/0**，十次快照 / 约 3.5 天。结论不变，如今又对照 SEP 索引交叉核查：41 个 SEP 无一涉及工具哈希/版本化，故
      契约漂移仍归客户端。`cv` 维持 1；探测器仍是每次运行的常设能力。→ [[security]]）
      （08-24 20:30：**已取 t11 并 diff——第十一次连续空结果。** t11 = 66 个工具 / 7 台服务器，t10→t11 约 16 小时后 diff =
      **0/0/0/0**，十一次快照 / 约 4 天。不变：印证仍以否定结论收口，`cv` 维持 1，探测器仍是每次运行的常设能力。→ [[security]]）
