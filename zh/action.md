...
- [~] **路由：传输层 vs 策略层之争** — MCP 的无状态核心 + `Mcp-Method`/`Mcp-Name` 头刚把路由*传输层*
      ...
      *agent 是谁*先标准化；*工具是什么*仍归客户端策略。ATProto Spaces 同一周采用 DPoP 绑定凭证——持有证明
      的第二次独立收敛。）
- [ ] **W3C 记忆 CG 会启动吗——又是否会触及语义字段？** AI Agent Memory Interoperability Community Group
      （2026-05-18 提议，「需 5 名支持者才能启动」）只圈定了*密码学信封*（单元形态、ML-DSA-65 身份绑定、逐单元加密、
      审计锚、共享/撤销、擦除），而非作者/置信度/溯源字段名。观察：（1）它是否启动，（2）是否有任何 MCP SEP 或 AAIF
      接手语义字段这一半，（3）类型化往返格式（engram pack / `.plur` capsule）是否获得第二个独立实现者——这是任何记忆
      规范的 `cv ≥ 1` 检验。→ [[agent-stack]]
