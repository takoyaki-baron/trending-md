...
- [~] **路由：传输层 vs 策略层之争** — MCP 的无状态核心 + `Mcp-Method`/`Mcp-Name` 头刚把路由*传输层*
      ...
      *agent 是谁*先标准化；*工具是什么*仍归客户端策略。ATProto Spaces 同一周采用 DPoP 绑定凭证——持有证明
      的第二次独立收敛。）
- [ ] **类型化记忆往返——会有第二个实现者吗？** W3C 记忆 CG 的信封规范已启动（2026-06-03），但语义字段这一半仍
      无人认领；唯一能改变这一点的是：类型化往返格式（Engram 的 PLUR pack / `.plur` capsule）获得*第二个独立*实现者——
      这是任何记忆格式的 `cv ≥ 1` 检验。观察：（1）类型化 pack 的第二个采用者，（2）是否有任何 MCP SEP 或 AAIF 工作组
      接手作者/置信度/溯源字段名。→ [[agent-stack]]
### 系统 —— 自我迭代
