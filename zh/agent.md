...
2. **Agent 安全是最直接的攻击面——而每一个被命名的类别最终都无人执行。** 每一个 MCP 服务器、
   ...
   **十六种反复出现的形态**，各有一例典型——常驻凭证跳板、补丁即逆向、默认暴露面、AI 辅助攻击、
   ...
   却无人执行——OWASP ASI05、工具调用边界、评估沙箱、MCP 工具钉扎。
      - **08-16→09-11 —— 十六种形态逐一填满；利用时差转为负数；补丁本身成为攻击面；第三方 ASM 扫描泄漏到共享基础设施；绕过系列落地；AI 服务代理是皇冠宝石级盒子；回环地址不是信任边界；Orval 元数据滞后复测（33 条公告中 0 条带 first_patched_version，如今是双向的）；GreyNoise 测得 agentic 攻击时钟 <4 小时，并让 PaperCut 从空工作区到首个受害者 RCE 以战役规模实现；Anthropic 九月威胁报告；Datasette 的双人前沿模型审计（已合并；全部详情 → [[security]]）。**
...
3. **本地推理正在被 MoE 稀疏性 + 磁盘流式加载解锁，而非量化。** kimi-k3-in-c、TurboFieldfare、
   ...
   应用。→ [[edge-inference]]
      - **08-21→09-14 —— 地基落定、浏览器是零安装端、无信号驱逐获得验证、本地语音成为单应用品类（已合并；全部详情 → [[edge-inference]]）：** colibri 用 *无 GPU* 从 NVMe 流式加载 744B MoE 的约 19,456 个路由专家；slotstream/Tiel-Coder 分裂了 SSD 流式学派（字节级一致解码作为 CI 测试）；M4 Pro 消费者蓝图；Apple M6 2nm + M5 Ultra 512 GB/1.2 TB/s；QAH 4-bit 胜过 bf16；mlc-ai/web-llm（权重从不离开标签页）；Random Attention（均匀随机驱逐追平最佳学习驱逐器——挑战者量到了在位者的零采纳）；VoiceStudio（一个 Tauri 应用 + MCP 打包 16 个 TTS + 11 个 ASR，CC-BY-NC 权重是许可陷阱）；CUDA-for-AMD-Windows 不带许可证文件。
