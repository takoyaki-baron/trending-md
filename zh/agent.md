...
1. **Agent 基础设施正在成为新的云——单体 CLI 正在分解为三个可分离的层次，各自在数周内诞生开源赢家。**
   ...
   - **09-16 —— 编排拒绝让模型自己阅卷；agent 扩展自己的 UI、拿到设备农场与受治理的测试世界（已合并；全部详情 → [[agent-stack]]）：** Ordewell 的 VerdictEngine（"模型永远不是仲裁者"）；greentfrapp/panel 的 agent 自建面板；vphone-cli 的 iPhone IPSW 虚拟设备农场 + MCP 服务器；Datamimic CE 对治 agent 伪造夹具的失败模式。
...
2. **Agent 安全是最直接的攻击面——而每一个被命名的类别最终都无人执行。** 每一个 MCP 服务器、
   ...
   **十六种反复出现的形态**，各有一例典型——常驻凭证跳板、补丁即逆向、默认暴露面、AI 辅助攻击、
   ...
   - **09-16 —— 生产级规模的"2013 年代"错误；更新通道本身成为武器（已合并；全部详情 → [[security]]）：** Baseten 镜像 Docker 层里的 PAT、vCenter 打补丁两个月后遭勒索软件利用、marimo 的 8 秒利用链、被二次植入的 WP 扩展、Delinea 金库 SAML 伪造、日本数字厅的已知漏洞失守、Cloudflare 审计技能作为防御镜像。
