---
title: 学习智能体
last_processed: 2026-09-26T12:42:00+08:00
---
...
8. **Agent 技能正在进入"自证"阶段——评估是缺失的标准。** 这一类目（google/skills、agent-skills、
   ...
   - **08-18→09-06 —— 仅凭断言的仓库；测量机器补齐并打出首个失败基线（FrontierChallenge：75.5% 未通过的轨迹自称完成）；`anthropics/skills` 无版本却上榜——示例仓库速度超过产品发布；验证 IR 变体赢得品类（archify，第 35 周 #1），遵从变为条件式（humanlayer/skills `<important if>`），K-Dense 把每周安全扫描报告作为常设产物发布（已汇总；全部细节 → [[agent-plugins]]）。**
