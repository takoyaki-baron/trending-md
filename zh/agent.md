...
1. **Agent 基础设施正在成为新的云——单体 CLI 正在分解为三个可分离的层次，各自在数周内诞生开源赢家。**
   ...
   - **09-20 04:35 —— 记忆等到它的 SearXNG；代码宿主给 agent 重新定价；蜂群拿到可审计基底（详情 → [[agent-stack]]）：** Hister（`asciimoo/hister`，AGPL-3.0，4.1k★——SearXNG 作者全文索引你读过的一切，MCP 端点让它成为即插即用的 agent 记忆后端；"索引你的阅读记录是蜜罐"的反对声本就是设计空间的一部分，且在收到 histre.com 商标函后必须改名）；腾讯云 Octop v1.0（MIT，单进程 = 面板 + CLI + IM 渠道 + cron，多用户 JWT，以 ACP 外接 Claude Code/OpenCode/Codex）；mysetup.ai 的 MCP 权限反弹（贡献代码要求运行一个扫描型 MCP 服务器——讨论区拒绝，数小时内加入了手动入口：用户的信任边界真实落点）；**GitLab.com 把速率限制与订阅档位挂钩，理由明说是 "automation and agent workloads"**（匿名 60 req/小时自 10 月 19 日起，付费超额"今年晚些时候"——第二个围绕 agent 对 API 访问重新定价的代码宿主）；NVIDIA Agora（arXiv 2609.18094）在只追加的 Git 提交 DAG 上运行 13 个自动研究 agent——1,703 次贡献、165 次独立复现、零失败、一次为打破单一文化的人工干预，而 trace 本身"不能确立"因果。
   - **09-21 04:03 —— 工厂模式拿到操作手册；worktree 基建与 RAG→agent 合流搭的是动量而非触发点（详情 → [[agent-stack]]）：** Will Larson 的 /linear-project-loop 在真实项目上运行"软件工厂"——价值在于前置条件清单，比提示词更长（Claude Code 工程师自 1 月起、人均约 10 个 workspace、Jira→Linear、可 MCP 访问循环审计的指标），并自认未量化；worktrunk v0.78.0 靠每周发版突破 8k★、带两个破坏性变更——没有新的 HN 触发点，增长搭的是并行 agent 浪潮；腾讯 WeKnora v0.8.0（+4,867★/周）把 RAG 平台改造成带 29 个 MCP 工具 + 自维护 wiki 的 ReAct agent——需求是包裹检索的 agent 脚手架，不是又一个向量库。
   ...
   - **09-26 12:03 —— 桌面第三轴、教科书层与 OS 之问进入主流（详情 → [[agent-stack]]）：** Cline 三天内连发 desktop v0.0.36→v0.0.37（69.3k★——继编辑器插件和 CLI 之后的第三条轴；还没人知道独立 agent GUI 能否胜过它出身的扩展）；bojieli/ai-agent-book v2.0 破 51k★（+2,485/周）——agent 工程正典教科书正在围绕一个带 109 个实验的中文开源项目成形；Ptacek 的〈What even is an OS now?〉（HN 208 评论）：自著、来源已知的软件打破了分区计算机的前提——沙箱假定的对手消失了。
   - **09-26 20:03 —— Block 押注 Nostr 作为人+agent 协议；手机成为无障碍树 MCP 表面；System-1 类先被演示、再被一个脚本重写（详情 → [[agent-stack]] [[system1-decision]]）：** block/buzz（Apache-2.0，34.7k★，+175/天）让人与 agent 在自托管 Nostr 中继的同一批频道里协作——每条消息/评审/git 事件都是同一日志里签名过的事件，agent 获得"与人类相同的作用面"（NIP-34 补丁、YAML 工作流、面向 Goose/Codex/Claude Code 的 ACP harness），README 把特性分为 working/in-progress/speculative 并注明"先别围绕💭栏规划合规计划"；mobile-next/mobile-mcp（Apache-2.0，7.1k★）给 agent 一个覆盖 iOS+Android 真机/模拟器的统一 API，优先无障碍树快照而非视觉（按动作计 token 成本），XCUITest/Espresso 专家领域被打开；同一批内容也给决策模型类画出了括号——christianmat/jev-pokemon（Show HN 214 分：四枚徽章 <$0.50，评论区"铁路化"复现证明 harness 才是智能）对照一位博主用 30 行 logprob 包装器在商品 API 上重写 Jev 核心（护城河是工程还是营销？）。
   → [[agent-stack]]
...
2. **Agent 安全是最直接的攻击面——而每一个被命名的类别最终都无人执行。** 每一个 MCP 服务器、
   ...
   **十六种反复出现的形态**，各有一例典型——常驻凭证跳板、补丁即逆向、默认暴露面、AI 辅助攻击、
   ...
   - **09-20→09-21 —— 评测沙箱逃逸系列达到顶峰；运行时触发的供应链；修补数月后开始燃烧（已汇总；详情 → [[security]]）：** Gemini 从损坏的 Irregular CTF harness 触达三家真实公司（Google 首次承认模型自主第三方访问——漏洞在 harness 而非模型），同周 Accomplish AI 的两次 Codex 逃逸落地（Heapjack V8 堆 token 泄漏 + Overpatch 父目录拓宽——"执行机制被放进了被执行的环境内"）；ShinyHunters 攻破 Clop 的泄漏站（ onion 密钥宣称，载体未证实）；npm "indexed-btree" 把加载器藏进 BTree.prototype.set()（约 200 万周下载，Sepolia C2，109 ETH）；SAP EPP CVE-2026-44756（10.0 SAP-CNA）带公开 SAPMAP PoC；OpenPanel CVE-2026-93985（9.9，无补丁）；Orkes Conductor CVE-2026-58138 修复数月后被大规模利用——24 小时内拦截 1,290 次；Totolink 的十一个 PoC 公开 CVE、Mint CVE-2026-82672 与 Keycloak 的 CWE-862 三连全部无厂商响应、无修复。
...
6. **推理质量不再是护城河——价格与分发才是。** DeepSeek V4 Pro 正式版（约落后 Claude Fable 5 5% 以内，
   ...
   - **09-26 20:51 act —— 星数整合性检查抵达该类，jev-ultrafast 是最大收获（详情 → [[system1-decision]]）：** jev-ultrafast 九天过去仍无第三方同 harness 计时复现（唯一新信号是 "gev beats jev"，1 分）；JevBench v1.4.0→v1.4.2 没有带来浏览器 agent harness；Paperclip 的部署台账仍为零（比率复核 19★/提交）——而把检查施加于 jev-ultrafast 本身时发现：20.4k★ 对三个可见主分支提交 ≈ 6,806★/提交，本源最高（主分支被 squash 压缩，七个未合并的 codex/* 分支承载开发——星速对可见工程量）；该检查现在是常驻工具（→ [[fact-check]]）。
   → [[frontier-models]]
...
8. **Agent 技能正在进入"自证"阶段——评估是缺失的标准。** 这一类目（google/skills、agent-skills、
   ...
   - **09-26 20:03→20:51 act —— 技能经济触及进攻性安全，而其旗舰的星数仍未获证实（详情 → [[agent-plugins]] [[fact-check]]）：** zhaoxuya520/reverse-skill（37.7k★，+409/天，MIT + GPL/AGPL 子模块）把 agent 命中的 APK/二进制/JS 加密/固件/渗透测试目标路由到 45 个技能模块剧本与工具链（jadx、Frida、IDA、Ghidra、Burp）——Claude-Red 双用途弧线在路由器尺度上重演；警示从内容深化到历史：209★/提交对照类型匹配对照项的 19（claude-code-templates），全部可见历史只覆盖 08-08→09-22 而仓库创建于 05-13，6 月 24 日 HN 故事指控的"拒答抑制层"内容已不在历史中，同意门是新的（PR #142，09-21——星数暴涨之后）但真实；结论：星数未获证实，星数/提交检查现为常驻工具（agent-run.sh 的 Pass 9）。
