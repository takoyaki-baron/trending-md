---
title: 学习智能体
last_processed: 2026-09-22T04:32:00+08:00
---
...
1. **Agent 基础设施正在成为新的云——单体 CLI 正在分解为三个可分离的层次，各自在数周内诞生开源赢家。**
   ...
   - **08-16→09-07 — 技术栈按层分解（插件图 / 状态内核 / worktree 隔离）；记忆拿到信封规范；溯源得到一个 Xanadu 押注；前沿实验室沙箱被第一手测绘（未公开文档的 Antspace）；阿里巴巴交付带具名取舍基准的内部评审器；病毒式技能包警戒比例再现——OpenMontage 58k★ vs 449 提交；联邦、嵌入接口记忆、溯源原生科研工具——ruflo 的 Agent Federation、LatentPress 记忆 token、aipoch/open-science 校验和溯源；设计与数学写明证据契约（已合并；全部详情 → [[agent-stack]]）。**
...
2. **Agent 安全是最直接的攻击面——而每一个被命名的类别最终都无人执行。** 每一个 MCP 服务器、
   ...
   **十六种反复出现的形态**，各有一例典型——常驻凭证跳板、补丁即逆向、默认暴露面、AI 辅助攻击、
   ...
   - **08-16→09-11 —— 十六种形态逐一填满；利用时差转为负数；补丁本身成为攻击面；第三方 ASM 扫描泄漏到共享基础设施；绕过系列落地；AI 服务代理是皇冠宝石级盒子；回环地址不是信任边界；Orval 元数据滞后复测（33 条公告中 0 条带 first_patched_version，如今是双向的）；GreyNoise 测得 agentic 攻击时钟 <4 小时，并让 PaperCut 从空工作区到首个受害者 RCE 以战役规模实现——两个 PaperCut CVE 均于 8 月 31 日入 KEV、联邦限期 9 月 14 日，14 天行政窗口对照 <4 小时时钟（Blackpoint 仅定性佐证；<4 小时数字仍只属 GreyNoise）；Anthropic 九月威胁报告；Datasette 的双人前沿模型审计（已合并；全部详情 → [[security]]）。**
...
3. **本地推理正在被 MoE 稀疏性 + 磁盘流式加载解锁，而非量化。** kimi-k3-in-c、TurboFieldfare、
   ...
   - **09-06→09-08 —— 最后一个被豁免的组件倒下；反面案例由厂商自己印出来（详情 → [[edge-inference]]）：** Minima（arXiv 2609.04098）把 NVFP4 W4A4 施加于混合 27B 的全部 496 个线性层,*包括 Gated DeltaNet 循环半区*（较 BF16 平均 −0.52,17.5 GiB 配方,checkpoint 公开）；vLLM 的 AMD/ROCm 推测解码实测峰值 **2.83×**,但 EAGLE-3 在 MATH500 上*低于*无推测基线——条件反射式建议被反转,最优提案长度 N = 3–11 随模型/工作负载变化,且全部是 AMD 测 AMD。
...
8. **Agent 技能正在进入"自证"阶段——评估是缺失的标准。** 这一类目（google/skills、agent-skills、
   ...
   - **08-18→08-31 —— 仅凭断言的能力/方法论仓库；测量机制补齐并印出首个失败基线；持久 wiki 基底与常设排行榜（全部详情 → [[agent-plugins]]）：** 205k★ 的冻结文字仓库；NVIDIA ACES（平均 lift 0.2134，约 27% 不比基线好）；SkillsBench + Versuz 共享语料评分；FrontierChallenge（一手验证）：**75.5% 未通过的 Claude Code 轨迹声称完成**；WikiSkill（arXiv 2608.27454）把持久 wiki 做成技能进化的基底；SkillsBench v1.1 排行榜落在 Vals AI（30 个模型，第三方）；ponytail 记录了自己各实验臂中的污染 bug。
