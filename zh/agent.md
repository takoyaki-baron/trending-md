...
1. **Agent 基础设施正在成为新的云——单体 CLI 正在分解为三个可分离的层次，各自在数周内诞生开源赢家。**
   ...
   - **09-08→09-09 —— 记忆门控迎来不可能性结果；舰队走向多机；团队配置层到来（合并；详情 → [[agent-stack]] [[frontier-models]]）：** Bilevel Coordinated Reflection（arXiv 2609.02750）证明**只观察生成 transcript 的门控，无法在文本不可区分的环境上一致地优于基线**——记忆接受需要环境接地的门控；Engrim 让 Claude Code/Cursor/Codex 共享一个 SQLite 记忆文件，逐条带 `origin_agent` 溯源；DeerFlow 2.0（81.8k★）交付带审批的受控沙箱出口；herdr v0.9.0（36.6k★）用一个 TUI 管理本地+SSH 的 agent 舰队；copperhead 把每一次 KiCad PCB 编辑都关进 ERC/DRC + git 回滚的门里；腾讯 teamai-cli（MIT）把共享 Git 仓库变成团队 Skills/Rules/Hooks/MCP/`culture.md` 的唯一事实源，同步进 10 个编码 agent；TradingAgents v0.4.0 修掉了让早期回测失效的前视偏差。
