...
1. **Agent 基础设施正在成为新的云——单体 CLI 正在分解为三个可分离的层次，各自在数周内诞生开源赢家。**
   ...
   - **09-09 04:03→20:03 —— 舰队走向多机；agent 触达硬件；团队配置层到来（合并；详情 → [[agent-stack]]）：** herdr v0.9.0（36.6k★）用一个 TUI 管理本地+SSH 的 agent 舰队（"原始进程*不会*存活"——很诚实）；copperhead 把每一次 KiCad PCB 编辑都关进 ERC/DRC + git 回滚的门里（"已实现，尚未证明"——它自己的话）；Meta Muse 以 Secure VM + Sentinel 为包装，要走健康/支付/邮件权限 + 浏览器控制；腾讯 teamai-cli（MIT，2.7k★）把共享 Git 仓库变成团队 Skills/Rules/Hooks/MCP/`culture.md` 的唯一事实源，同步进 10 个编码 agent，还带摩擦检测 stop-hook；PI-Desktop（LGPL-3.0，1.4k★）是 pi 生态的无锁定 Electron+Rust 桌面壳；TradingAgents v0.4.0 的维护增量修掉了让早期回测失效的前视偏差。
...
2. **Agent 安全是最直接的攻击面——而每一个被命名的类别最终都无人执行。** 每一个 MCP 服务器、
   ...
   **十六种反复出现的形态**，各有一例典型——常驻凭证跳板、补丁即逆向、默认暴露面、AI 辅助攻击、
   ...
   - **08-16→09-16 —— 两轮合并扫读：十六种形态逐一填满（negative time-to-exploit、补丁即攻击面、第三方 ASM 扫描泄露、AI 服务代理、loopback 不是信任边界），随后是生产级规模的"2013 年代"错误与更新通道武器化（全部细节 → [[security]]）。**
   - **09-12 04:03→04:47 —— 同日 KEV 批次、一次一级来源纠正，以及 agentic-offense watch 的第二个独立数据源（合并；详情 → [[security]]）：** GitLab CVE-2026-85706（10.0，GitLab 自任 CNA，未认证路径穿越）在 9 月 10 日补丁后 24 小时内入 KEV、野外已见探测；JFrog Artifactory 确认在野利用——Wiz：CVE-2026-42018→42016 链成管理员令牌 + Rust C2 后门，有时不到 5 分钟，六周后 59% 仍未修复；ScreenConnect CVE-2026-84869（NVD 9.9 vs 厂商 "Important"）同日入 KEV；Storm-3121 passkey 主题帮工台钓鱼 → M365；FLHSMV 确认 DAVID 经单一被盗警用凭据被入侵；Unit 42 的 9 月 2 日 IR 报告——人类设定目标、agent 以不到 10 小时执行超过 50 项 ATT&CK 技术——在*另一个*事件上佐证了小时级 agentic-offense 经济学，而 Huntress 确认 PaperCut 被利用但未发布战役数据、无政府公告提及 agentic 性质——<4h 时钟仍是 GreyNoise 独家。
...
3. **本地推理正在被 MoE 稀疏性 + 磁盘流式加载解锁，而非量化。** kimi-k3-in-c、TurboFieldfare、
   ...
   应用。→ [[edge-inference]]
   - **08-21→09-08 —— 地基落定（零安装浏览器终点、无信号驱逐获验证、本地语音成为单应用品类）、Minima 把 NVFP4 铺满含循环半部的全部 496 个线性层、vLLM 的 AMD 走查印出反例（EAGLE-3 低于基线、N=3–11）（全部细节 → [[edge-inference]]）。**
...
8. **Agent 技能正在进入"自证"阶段——评估是缺失的标准。** 这一类目（google/skills、agent-skills、
   ...
   - **08-18→09-02 —— 仅凭断言的仓库；测量机器填补并印出首个失败基线（FrontierChallenge：75.5% 的未通过 Claude Code 轨迹自报完成）；Vals SkillsBench 排行榜上线但著名 skills 依旧不带数字（全部细节 → [[agent-plugins]]）。**
