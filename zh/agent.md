...
1. **Agent 基础设施正在成为新的云——单体 CLI 正在分解为三个可分离的层次，各自在数周内诞生开源赢家。**
   ...
   - **08-16→09-09 —— 技术栈按层分解（插件图 / 状态内核 / worktree 隔离）；记忆先拿到信封规范，随后拿到针对纯转写门控的 Bilevel Coordinated Reflection 不可能性结果；出处拿到 Xanadu 押注；前沿实验室沙箱被第一手测绘（未公开文档的 Antspace）；阿里巴巴带注释权衡基准地开源内部评审器；病毒式技能包警告比率再现（OpenMontage 58k★ vs 449 commits）；ruflo 的 Agent Federation、LatentPress 记忆 token、aipoch/open-science 校验和出处；设计与数学各自亮出证据契约；编排层拒绝自我评分（Ordewell VerdictEngine）；机群走向多机（herdr）；团队配置层到来（腾讯 teamai-cli）（已合并；全部细节 → [[agent-stack]] [[frontier-models]]）。**
...
2. **Agent 安全是最直接的攻击面——而每一个被命名的类别最终都无人执行。** 每一个 MCP 服务器、
   ...
   **十六种反复出现的形态**，各有一例典型——常驻凭证跳板、补丁即逆向、默认暴露面、AI 辅助攻击、
   ...
   - **08-16→09-16 —— 两轮合并式盘点补齐十六种形态（负的时间到利用、补丁后逆向、第三方 ASM 扫描泄露、AI 服务代理、回环不是信任边界），随后是生产规模的 2013 年代错误；同日 KEV 批次（GitLab 10.0 修补后 <24h 进 KEV、JFrog Artifactory 证实在野链式利用、ScreenConnect NVD-9.9）、一次一手来源纠正、agentic 攻击观察的第二张独立网格；邮件列表与测试服务器成为初始访问面（Trezor × Brevo，约 34.7 万钓鱼）；绕过 Mullvad VPN kill switch 的硬件 keepalive（已合并；全部细节 → [[security]]）。**
...
6. **推理质量不再是护城河——价格与分发才是。** DeepSeek V4 Pro 正式版（约落后 Claude Fable 5 5% 以内，
   ...
   - **09-26 04:35 —— System-1 层迎来本地运行器（详情 → [[system1-decision]]）：** Ollaya（ollaya-dev/ollaya，Rust，Apache-2.0，经 API 核验 91★，Show HN 129 分）以 Ollama 风格命令在本地服务 Jev 兼容的单次前向决策模型——约 3 MB ONNX 图、对从原作者 HF 仓库拉取的权重做 sha256 校验（不转存）、RTX 4090 上五问 8–10 毫秒、带 Claude Code/Cursor 的 MCP；HN 反驳有实质内容：Ollama 随时可以自己加决策模型支持，且旗舰示例“基本上就是分类”。
   → [[frontier-models]]
...
8. **Agent 技能正在进入"自证"阶段——评估是缺失的标准。** 这一类目（google/skills、agent-skills、
   ...
   - **08-18→09-05 —— 仅凭断言的仓库；测量机器补齐并打出第一个失败基线（FrontierChallenge：75.5% 未通过的 Claude Code 轨迹自述完成）；Vals SkillsBench 排行榜落地但著名技能不带数字；`anthropics/skills` 以约 512★/天登上 #5 而无任何发布——示例仓库的速度超过产品发布（已合并；全部细节 → [[agent-plugins]]）。**
