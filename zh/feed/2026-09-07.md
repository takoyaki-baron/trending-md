...
[`🔗 Sansec:StyleSmuggler`](https://sansec.io/research/stylesmuggler) · [`🔗 The Hacker News 报道`](https://thehackernews.com/2026/09/unpatched-magento-and-adobe-commerce.html)
...
- **Velocity:** ▮▮ rising
- **Source:** GitHub Trending · 3,215 stars · 今日 +436 stars · TypeScript · GPL-3.0
...
Elastic 记录了四个此前未报告、与 REVSTEALER 关联的程序——REVSTEALER 是商业 Windows 窃密木马(VirusTotal 约 4,700 个样本匹配)——它们在窃密木马自删除后依然持久存在:ProManager(钱包窃取 + 覆盖层钓鱼 + 按键记录)、WinUpdate(剪贴板加密货币地址替换 + 助记词捕获)、SoftManager(反向代理中转)与 LockAppHost(CMSTP 提权、Defender 排除项、禁用 5 个 Windows Update 服务与 11 个计划任务、把矿机藏在挂起的 `nslookup.exe`/`svchost.exe` 里)。分发渠道是 ≥17 个被劫持的 YouTube 频道，用 AI 生成视频推送游戏外挂诱饵；"约 2026 年 2 月起售卖"的定年(最早 VT 样本)与假的"Claude Opus 5 Free Desktop"诱饵是 Morphisec 的发现、经 The Hacker News 报道流转，并不在 Elastic 报告原文中(无迹象表明 Anthropic 被入侵)。

**Why it matters:** 自删除窃密木马 + 持久化模块的组合意味着清理后主机"看起来干净"，而 Defender 排除项和矿机仍然存活——响应者必须重新启用服务并轮换会话，而不是只改密码。关键注意项(出自 The Hacker News 对报告的转述，并非 Elastic 页面原文):Elastic 从未观察到四个模块中的任何一个被投递到活跃 REVSTEALER 主机上——关联基于共享手法而非观察到的交接——且公开 YARA 集没有 LockAppHost 规则。
...
一个 Electron/React/Prisma-SQLite 桌面工作台，封装可选的智能体后端(Claude Code、OpenCode、Codex、CodeBuddy)，配 Python/R 笔记本、22 个精选科学技能(AlphaFold2、Boltz、DiffDock、ESM-2、scGPT、Remote Compute SSH)与 24 个科研连接器(PubMed、bioRxiv、ChEMBL、Clinical Trials)。差异点是溯源:每个产物都是不可变的带校验和版本，绑定到产出它的代码、执行历史、环境清单与确切的对话分支——无法验证的证据会被显式标记为不可用。附带 CLI 与无头 SDK。
...
## 26. NX 位不只是安全——一个裸机调试故事:NX 正是挡住推测指令取指的那道闸

- **Velocity:** ▮ steady
- **Source:** Hacker News · 35+ pts · 23 条评论 · ~2d 前 (9 月 5 日 ~10:00 UTC+8)
- **Tags:** `nx-bit` `cpus` `arm` `memory-safety` `systems`

purplesyringa.moe 的客座文章完整记录了在联发科 MT6735(Cortex-A53)上为 postmarketOS 调试裸机 ARM64 hypervisor 的过程:启用 `CTR_EL0` 拦截后,手机会随机死机。追查终点是 `blr x0`(动态、可误预测)与 `bl`(静态)的区别:CPU 的推测*指令*取指——而非数据访问——抵达了地址 0x0,即已被锁出的 bootrom;把这些页标记为不可执行才将其挡住。文章引用 ARM 官方文档给出的结论:Device 内存只阻止推测*数据*访问,因此在 ARM 上 NX 位"根本与安全无关——它本质上只是一个属性位"(作者检索了 Cortex-A53 勘误表,未发现任何相似条目——行为符合规范,不是硅片缺陷)。
