...
## 17. DaVinci Resolve 21.1——Blackmagic 带来 100+ 新工具，真正的 AI 头条是助手驱动剪辑器

- **Velocity:** ▮▮ rising
- **Source:** Blackmagic Design 发布公告 9 月 8 日（已读全文） · HN 367+ pts · ~14h 前 (~22:30 UTC+8)
- **Tags:** `davinci-resolve` `video` `fusion` `release` `ai-tools`

Blackmagic 于 9 月 8 日发布 DaVinci Resolve 21.1:剪辑、调色与 Fusion 页面共新增超过 100 个工具与控件，Fusion 中 25 个基于 Krokodove 的形状与 3D 工具，多机位切换扩展到 25 路，Dolby Vision/HDR10+/HDR Vivid 各自独立的 MultiMaster 调色修剪，以及 20 个新脚本 API。**就地更正（2026-09-09 20:31 轮）：**发布稿原文写明真正的 AI 头条是 **AI 助手集成——Resolve 现已支持 Claude、Claude Code 与 ChatGPT Codex** 以对话方式驱动项目分析、媒体整理、精彩集点剪辑与批量渲染。此前的表述（"Neural Engine 媒体检索、场记板识别、去龄、去瑕"）在原始发布稿中并不存在，系二手转述，予以撤回。速度保留：HN 的热度附着在真实的发布上，而非被撤回的那部分。

**Why it matters:** 智能体 AI 时代 Resolve 的第一次大版本更新，以 *agent 集成*的形态落地——主流 NLE 把工作流开放给第三方编码助手，这比 Neural Engine 小升级是更锋利的 agent 分发数据点。更正本身也是教训：厂商自己的自由文本发布稿几秒钟就能核验，一核验故事就从"付费层 ML 功能"翻转成"你的编码 agent 可以操作你的剪辑器"。
