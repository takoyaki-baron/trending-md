...
## 33. "我说了不，Apple 说了行" — macOS 升级把用户明确关闭的 Apple Intelligence 重新打开

- **Velocity:** ▮▮▮ trending
- **Source:** Hacker News · 303+ pts · 225 comments · ~4h ago (~16:04 UTC+8)
- **Tags:** `apple` `apple-intelligence` `privacy` `consent`

David Bushell 的升级日记录：此前在 macOS 15.3 自动开启一个每 15 分钟回传个人数据的
功能后，他曾明确关闭 Apple Intelligence 和 Siri（"一次相当明确的'不'"）。本周从
macOS 15 升到 27，他发现 AI 功能被重新打开，关闭开关也不见了。Siri "已禁用"却仍有
多个杀不掉的 Siri 进程占用内存并写数据；Apple Intelligence 占了 22.28 GB 磁盘（按
Apple £500/TB 的存储升级定价折算约 £11）；Screen Time 里的隐藏绕过方法只是把 AI
功能从菜单里藏起来，并没有真正停用。

**Why it matters:** 这正是第 30 条留下的问题的具体答案——"关"到底是不是真的关。
Apple 本周发布了真正的逐项开关，但既有的 opt-out 显然没能挺过升级——所谓同意是
按版本算的状态，不是设置。注意：这是单个用户的亲历记和观点文章——但每一个细节
（进程、存储、Screen Time 路径）都可以在那台机器上核实。

[`🔗 dbushell.com`](https://dbushell.com/2026/09/22/apple-intelligence/) · [`🔗 HN 讨论`](https://news.ycombinator.com/item?id=49797982)

---

## 34. agent-substrate/substrate — "默认安全的 agent 执行运行时"，今日未被覆盖的最快上升者（+498/天）

- **Velocity:** ▮▮▮ trending
- **Source:** GitHub Trending · 2.7k stars · +498/day
- **Tags:** `agent-infra` `kubernetes` `sandboxing` `gvisor`

一个 Go 编写的运行时（Apache-2.0），把大量大部分时间空闲的 agent（"actor"）复用到
较小的常驻 worker 池上，构建在 Kubernetes 之上：gVisor 与 microVM（cloud-hypervisor）
沙箱后端，全状态快照实现挂起/恢复（"Actor Teleport"），休眠期间文件系统与内存可持久
化，请求停靠，以及带 MITM 拦截的出口策略。支持 ADK、LangChain、Claude Code、Codex
与 MCP 服务器。宣称：密度为标准容器运行时的 10 倍、500+ 次/秒挂起/恢复下恢复时间
低于 500ms、8 个 Pod 跑约 250 个 actor（"30 倍以上超售"）——全是厂商自述；仓库链接
了基准测试指南，但页面上看不到方法论。

**Why it matters:** 用超售换 agent 密度正在成为本季的基础设施命题，这是目前最具体
的开源实现。README 自己的警告是对冲："不在生产就绪状态，API 几乎肯定会变"；"不是
Google 官方支持的产品"，且被排除在 Google 开源漏洞奖励计划之外；仅支持最新 Kubernetes
stable 加一个 minor 版本。

[`🔗 GitHub 仓库`](https://github.com/agent-substrate/substrate) · [`🔗 GitHub Trending`](https://github.com/trending?since=daily)

---

## 35. JetBrains 发布 Air — 覆盖 IDE、Web、CLI 与移动端的统一 agent 系统

- **Velocity:** ▮▮ rising
- **Source:** JetBrains / Hacker News · 39+ pts · 26 comments · ~1h ago (~19:00 UTC+8)
- **Tags:** `jetbrains` `agents` `ide` `product-launch`

Air 是"一个用 agent 构建软件的系统"，覆盖四个界面（IDE、Web、CLI、移动端），集成
Claude Agent、Codex、Junie、Copilot、OpenCode，"以及任何可通过 ACP 接入的 agent"。
组件包括：IDE 内的 Air（通过注册表自动发现本地 agent、diff 审查、行级评论让 agent
执行）、Air Teams（专属云环境、自动化、集中 MCP 配置）、Air Governance（组织级权限），
以及由 merge/PR/push 触发、按 AI 积分记账的自动化。云运行"已对部分客户开放"，逐步
推广中；IDE 插件处于 alpha；可自带 API key，或按公开 API 价格计费的 JetBrains AI
积分。

**Why it matters:** 去年 12 月 JetBrains 放弃 Fleet 押注 Air，这就是落点——值得注意
的选择是它做的是 agent 无关的基础设施（为别家 agent 提供控制面与审查界面），而不是
又一个绑定自家 IDE 的 agent。注意：完整定价尚未公布，而且发布才一小时，讨论热度
还很低。

[`🔗 jetbrains.com/air`](https://www.jetbrains.com/air/) · [`🔗 HN 讨论`](https://news.ycombinator.com/item?id=49799287)

---

## 36. gzip 能当语言模型吗？一个零训练生成器给出答案："勉强算？"

- **Velocity:** ▮▮ rising
- **Source:** Hacker News · 196+ pts · 78 comments · ~6h ago (~14:08 UTC+8)
- **Tags:** `compression` `information-theory` `llm` `education`

nathan.rs 作者的 `gzipt`（纯标准库 Python）：先用语料填充 DEFLATE 的 32 KiB 滑窗，
再以 `len(compress(context + candidate))` 为候选续写打分——越短代表"越被预测到"。
两个技巧让它勉强能跑：对多字节跨度做束搜索（gzip 只输出整数字节数，逐字节步进
常常打平、信号淹没在量化噪声里），以及打分上下文只保留最后 `tail` 字节——DEFLATE
偏爱便宜的近距匹配，完整历史会塌缩成逐字自抄循环。莎士比亚续写样本出来是认得出
的剧本格式加乱码；作者自己的结论是"勉强算？"——并引用 DeepMind 的《Language
Modeling Is Compression》（2023），那篇论文的脚注早已记录 gzip 生成"效果很差"。

**Why it matters:** 一个诚实可跑的演示：零训练参数下压缩=预测等价性长什么样——也
是如何报告阴性结果的范本（不宣称任何基准， caveat 写在行文中）。相对 2023 年论文
的真正新意，是跨字节跨度的束搜索构造。

[`🔗 nathan.rs`](https://nathan.rs/posts/gzip-lm/) · [`🔗 arXiv:2309.10668`](https://arxiv.org/abs/2309.10668) · [`🔗 HN 讨论`](https://news.ycombinator.com/item?id=49797323)

---

## 37. browser-use/video-use — "用编码 agent 剪视频"拿下 25.5k stars

- **Velocity:** ▮▮ rising
- **Source:** GitHub Trending · 25.5k stars · +155/day
- **Tags:** `video-editing` `coding-agents` `browser-use` `mit`

把原始素材丢进文件夹，跟编码 agent（Claude Code、Codex）聊几句，得到 `final.mp4`
——不需要时间线软件。设计要点：LLM 从不直接看帧。它读一个约 12KB 的转写文件
（ElevenLabs Scribe：词级时间戳、说话人分离、音频事件），加上只在决策点按需生成的
缩略图序列 PNG，然后剪辑、调色、加 30ms 音频淡入淡出和烧录字幕；自带评估循环会复
查剪辑边界（最多 3 轮修复），叠加动画由并行子 agent 用 Remotion/Manim/PIL/
HyperFrames 完成。MIT 协议。

**Why it matters:** 用表示方式换 token 效率——文本转写充当视频的世界模型——正是本
月 harness 工程系列条目反复出现的模式，只是这次用在原始帧根本用不起的媒介上。
注意："4500 万 token 噪声"的对比是项目自己的说法；"100% 开源"的宣称与硬依赖付费的
ElevenLabs API key 并存略显尴尬；没有 release、21 个 commit、93 个开放 PR——而且我们
没找到新的发布事件，这波趋势是持续动能，而不是能点名的新触发器。

[`🔗 GitHub 仓库`](https://github.com/browser-use/video-use) · [`🔗 Releases`](https://github.com/browser-use/video-use/releases)

---

## 38. SharePoint CVE-2026-65660：微软公告写的是"欺骗、6.5 分" — 今天的分析证实是认证后 RCE

- **Velocity:** ▮▮ rising
- **Source:** Viettel / The Hacker News · ~1h ago (~19:17 UTC+8)
- **Tags:** `sharepoint` `microsoft` `rce` `cvss`

Viettel 的 Dinh Ho Anh Khoa——2025 年 Pwn2Own Berlin 的 ToolShell 研究者——今天
发布完整技术细节，证明 CVE-2026-65660 是 SafeControls 列表绕过：ToolPane 重建
`Register` 指令时把属性值写入双引号之间却不转义内嵌引号，认证后的攻击者得以在类型
检查后注册任意 .NET 类，再经 `XamlServices.Parse()` 反序列化到达代码执行（附内存马
payload）。它可与另一个已修补的认证绕过（6 月 9 日修复）串成预认证 RCE 链，但仅限
开启匿名页面访问的配置。8 月 11 日已修补，补丁还默认禁用了出漏洞的功能。暂无在野
利用报告，未上 CISA KEV；微软评估利用可能性"低"——但完整 exploit 标记已公开。Khoa
指出 SharePoint 2013（2023 年已 EOL）同样受影响；微软公告只列了 2016/2019/
Subscription Edition。

**Why it matters:** 评分拉锯本身就是这条新闻。微软公告长期显示 6.5/欺骗、无完整性
与可用性影响；CVE 记录现已改题为 RCE（CWE-94），当前 NVD 记录给出 CVSS 3.1
**8.8 AV:N/AC:L/PR:L/UI:N——CNA（微软）赋分，状态 Modified**。按公告做分级的防御
者看到的是一个中危欺骗漏洞，而不是接近满分的代码执行。
...
## 39. Meta Muse 的隐藏听写端点：Wardle 的 PoC 把这个助手变成后门

- **Velocity:** ▮ steady
- **Source:** The Hacker News · PoC 9 月 21 日，分析 9 月 22 日（~14:33 UTC+8）
- **Tags:** `muse` `meta` `macos` `agent-security`

Objective-See 的 Patrick Wardle 发布了针对 Mac 版 Muse 应用的 PoC：一个未公开的
偏好设置 `endo_voyager_dictation_endpoint` 决定听写内容的去向——而任何以当前登录
用户身份运行的程序无需额外权限即可改写它。在此基础上他演示了三件事：读取用户听写
内容、注入 Muse 信任并照办的指令、截获 Muse 的会话令牌——他用该令牌驱动了自己
iPhone 上的 Muse（位置上报、蓝牙扫描、智能家居指令列表）。保留的边界条件：需要已
有的代码执行，不攻破 macOS TCC/钥匙串保护，未证明 Meta 的云端隔离被打破，且他的
测试中 Muse 只起草消息而没有自行发送。他选择直接公开而未先报告 Meta；Meta 此后
推送了他所称的"修复"（未经证实，无安全公告）。与第 2 条的 Amazon 壁垒对峙是不同
的问题——目标相邻，漏洞各异。

**Why it matters:** agent 自己被授予的权限就是攻击面：恶意代码不需要提权，只需要
操纵一个已持有钥匙、签名合法的应用——而 EDR 可能不会对来自它的命令报警。带跨设备
会话的 agent 级应用，意味着一次 Mac 沦陷等于该账号触达的每台设备被接管。
...
## 40. Univer 更名"Office Harness for AI Agents" — agent 可工作树协作再合并的办公运行时

- **Velocity:** ▮ steady
- **Source:** GitHub Trending · 14.9k stars · +202/day
- **Tags:** `office` `agents` `sdk` `typescript`

DreamNum 的 Univer（Luckysheet 团队）把表格、文档、幻灯片、画布、关系型表——PDF
"即将推出"——统一进一个 Apache-2.0 运行时，提供无头 Node 模式、面向 Claude Code/
Codex/OpenCode 的 CLI，以及 `univer-mcp` 插件。agent 定位如今写得很明白：人与 agent
共编同一文件，agent 在隔离的工作树草稿中工作、经人工审查后再合并，git 式版本历史
记录每次变更，agent 还能自我验证——自己设定校验条件并迭代到通过。官网引用了
SpreadsheetBench 第一名的成绩（68.86%，人类基准 71.3%）——自报数据。

**Why it matters:** 办公套件正被重建成 agent 的验证面——结构化 API、截图与版式诊断
充当检查手段——这与编码 agent 评测的走向一致。嵌入前先看清边界：实时协作、导入
导出、打印、图表与数据透视表属于 Univer Pro 商业版，docs/slides 成熟度低于表格，
而 SpreadsheetBench 的数字是厂商自己的。

[`🔗 GitHub 仓库`](https://github.com/dream-num/univer) · [`🔗 univer.ai`](https://univer.ai/)

---

## 41. Treg："agent 工具版的 OpenRouter" — 一个令牌、2630 个端点、按次计费

- **Velocity:** ▮ steady
- **Source:** GitHub Trending · 2.0k stars · +197/day
- **Tags:** `agent-tools` `api-gateway` `credentials` `metering`

Treg（可自托管的 Python/FastAPI，托管于 treg.to）给 agent 提供统一的基础 URL 与
令牌来调用厂商工具 API——Semrush、Moz、SerpApi、Hunter、Crunchbase 等。agent 请求
的是能力而非具体工具；treg 路由到某个服务商，在服务端注入凭证，并展示各服务商实测
成功率与速度，让 agent 按证据选择。按次计费、无订阅（引用示例：Semrush 关键词查询
$0.006/次；宣称"零加价"）；团队可登记自己的 key、CLI 与 `SKILL.md` 配方在全组织
共享。

**Why it matters:** 工具调用层拥有自己的 OpenRouter 是真实未被满足的需求——凭证
注入加按次计量，正是调用厂商 API 的 agent 缺的东西。但在把密钥交给它路由之前，
先核对几处不一致：README 宣称 3000+ 端点、60+ 服务商、"Apache 2.0 附加条款"，
而托管站点写的是 2630 个端点、47 家服务商、AGPL。另注明：响应为计费留证最多缓冲
8 MiB、丢失 Fernet 密钥将导致已存密钥永久无法恢复、托管 CLI 默认向 PostHog 发送
遥测。

[`🔗 GitHub 仓库`](https://github.com/superdesigndev/treg) · [`🔗 treg.to`](https://treg.to/)

---

## 42. claude-code-templates 突破 30.9k stars — skills 经济的聚合分发层

- **Velocity:** ▮ steady
- **Source:** GitHub Trending · 30.9k stars · +33/day
- **Tags:** `claude-code` `skills` `aggregator` `mit`

davila7 的 claude-code-templates 是 MIT 协议的 CLI，用于配置与监控 Claude Code：
100+ 个 agent、斜杠命令、hooks、设置与 MCP 集成，经 `npx claude-code-templates@latest`
即可安装，另有实时分析面板、移动端会话监视器、健康检查与插件面板。它以保留署名的
方式聚合第三方合集——K-Dense 科学 skills（139 个）、Anthropic 官方 skills（21 个）、
wshobson/agents（48 个）、obra/superpowers 等。

**Why it matters:** 这是 skills 浪潮的分发层，也继承了聚合器固有、且值得直说的风险：
装入的内容沿用原作者的许可与质量，README 把赞助位（Bright Data、Z.AI、Neon、
Vercel）混进了目录。未发现新的发布事件——+33/天是稳定累积而非爆发，按此如实记录。

[`🔗 GitHub 仓库`](https://github.com/davila7/claude-code-templates) · [`🔗 GitHub Trending`](https://github.com/trending?since=daily)
